...
class TestBugTrackerIds(unittest.TestCase):
    ...
    def test_current_active_bugs_section_contains_current_release_blockers(self):
        ...
        release_drift = content.index("## ZNE-439 - repo froze 0.1.93")
        ...
        self.assertIn("should `026f189` / `v0.1.93` replace the documented `0.1.91` release target", active_section)
        self.assertIn("repo HEAD is now the frozen `026f189` / `v0.1.93` candidate", active_section)
        self.assertIn("`repo_head_commit: 026f189`", active_section)
        self.assertIn("unapproved `v0.1.93` freeze/tag", active_section)
