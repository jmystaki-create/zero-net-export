...
class TestBugTrackerIds(unittest.TestCase):
    ...
    def test_current_active_bugs_section_contains_current_release_blockers(self):
        ...
        self.assertIn("should `0b4f420` / manifest `0.1.94` replace the documented `0.1.91` release target", active_section)
        self.assertIn("resolves the component install candidate to `0b4f420` / manifest `0.1.94` after the post-freeze component guidance fix", active_section)
        self.assertIn("`preferred_validation_commit: 0b4f420`", active_section)
        self.assertIn("unapproved `v0.1.94` freeze/tag or post-freeze `0b4f420` component boundary", active_section)
