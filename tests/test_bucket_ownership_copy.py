...
class TestBucketOwnershipCopy(unittest.TestCase):
    def test_bucket_ownership_and_workspace_split_stay_visible(self):
        ...
        self.assertLessEqual(len(configure_sources_description.splitlines()), 7)
        self.assertIn("Select sensors for this service only.", configure_sources_description)
        self.assertIn("Missing: {missing_sources}", configure_sources_description)
        self.assertIn("Progress: {source_mapping_progress}", configure_sources_description)
        self.assertIn("If a valid dropdown choice is rejected", configure_sources_description)
        self.assertNotIn("Grid mode", configure_sources_description)
        ...
        self.assertNotIn("Blocking source repair", configure_sources_description)

        steps = strings["options"]["step"]

        init_step = steps["init"]
        init_description = init_step["description"]
        self.assertEqual(init_step["title"], "Zero Net Export guided setup")
        self.assertTrue(init_description.startswith("Choose the next native Home Assistant workflow"))
        self.assertIn("Guided workflows", init_description)
        self.assertIn("- Sensors: map source roles and repair source health.", init_description)
        self.assertIn("- Controls: tune target export, deadband, reserve, and refresh interval.", init_description)
        self.assertIn("- Managed Devices: add, review, enable, edit, or remove controllable loads.", init_description)
        self.assertIn("- Diagnostics: review blockers, install evidence, and repair hints.", init_description)
        self.assertIn("Recommended section: {recommended_section}", init_description)
        self.assertIn("Recommended path: {recommended_path}.", init_description)
        self.assertNotIn("Bucket ownership and paths", init_description)
        self.assertNotIn("Structured control board", init_description)
        self.assertNotIn("Command-center use", init_description)

        native_setup_description = steps["native_setup"]["description"]
        self.assertIn("Pick the grid sensor layout for this service.", native_setup_description)
        self.assertIn("Sensors now", native_setup_description)
        self.assertIn("- Missing required roles: {missing_sources}", native_setup_description)
        self.assertIn("- Progress: {source_mapping_progress}", native_setup_description)
        self.assertIn("- Issues: {source_blocker_summary}", native_setup_description)
        self.assertIn("Use Sensors for source roles and source health.", native_setup_description)
        self.assertNotIn("Bucket ownership and paths", native_setup_description)
        self.assertNotIn("Role guide for common real-world labels", native_setup_description)
        ...
        self.assertIn("Map the source roles for this service.", source_mapping_description)
        self.assertIn("Progress", source_mapping_description)
        self.assertIn("- Required roles mapped: {source_mapping_progress}", source_mapping_description)
        self.assertIn("- Missing roles: {missing_sources}", source_mapping_description)
        self.assertIn("- Blocking issues: {source_blocker_summary}", source_mapping_description)
        self.assertIn("Current health", source_mapping_description)
        self.assertIn("Fallback fields are only for valid entities Home Assistant will not accept in the selector.", source_mapping_description)
        self.assertNotIn("Role guide for common real-world labels", source_mapping_description)
        self.assertNotIn("Current source bindings, for cross-check only", source_mapping_description)
        self.assertNotIn("Bucket ownership and paths", source_mapping_description)
        ...
        self.assertNotIn("review the source map", native_support_source)
        self.assertIn("review the current source bindings", native_support_source)
        ...
        self.assertIn("Manage controllable loads for this service.", devices_description)
        self.assertIn("Connected devices", devices_description)
        self.assertIn("Use Managed Devices for add, review, enable, edit, and remove.", devices_description)
        self.assertIn("Detailed per-device review path:", devices_description)
        self.assertNotIn("Bucket ownership and paths", devices_description)
        ...
        self.assertNotIn("Managed devices (top section)", bulk_enable_description)
        ...
        self.assertIn("Secondary per-device ⚙ Settings review/audit path:", bulk_enable_description)
        ...
        self.assertIn("Secondary per-device ⚙ Settings review/audit path after promotion:", vetting_description)
        ...
        self.assertIn("Secondary per-device ⚙ Settings review/audit path after save:", add_description)
        ...
        self.assertIn("Path: {configure_path}.", source_description)
        self.assertIn("Path: {configure_path}.", source_mapping_description)
        self.assertIn("Next: {source_next_step}", source_description)
        self.assertIn("Next: {source_next_step}", source_mapping_description)
        ...
        self.assertIn("Set the service control policy.", policy_description)
        self.assertIn("Controls now", policy_description)
        self.assertIn("Decision: {control_decision_summary}", policy_description)
        self.assertIn("Outcome: {control_outcome_summary}", policy_description)
        self.assertIn("Next: {policy_next_step}", policy_description)
        self.assertIn("Use Controls for target export, deadband, reserve, and refresh interval.", policy_description)
        self.assertIn("Source repair stays in Sensors; fleet work stays in Managed Devices.", policy_description)
        self.assertIn("Path: {policy_path}.", policy_description)
        self.assertNotIn("Bucket ownership and paths", policy_description)
        self.assertNotIn("Primary path: {policy_path}.", policy_description)

        support_description = steps["support"]["description"]
        self.assertIn("Review diagnostics and blockers for this service.", support_description)
        self.assertIn("Diagnostics now", support_description)
        self.assertIn("- Top alerts: {support_attention_summary}", support_description)
        self.assertIn("Blockers", support_description)
        self.assertIn("Blocking source roles: {support_source_attention_roles}", support_description)
        self.assertIn("- Repair step: {support_source_repair_step}", support_description)
        self.assertIn("Candidate cues", support_description)
        self.assertIn("Install evidence", support_description)
        self.assertIn("Exact-build step", support_description)
        self.assertIn("Evidence path: {support_install_snapshot_path}", support_description)
        self.assertIn("Path: {configure_path}.", support_description)
        self.assertNotIn("Bucket ownership and paths", support_description)
    ...
    def test_active_project_docs_use_current_bucket_labels(self):
        ...
        self.assertIn("**Source-role diagnostics** — source-role status", docs["DASHBOARD_SETUP.md"])
        self.assertIn("first-class per-source diagnostics slice for each source role", docs["DASHBOARD_SETUP.md"])
        self.assertNotIn("**Source-level diagnostics** — mapped source status", docs["DASHBOARD_SETUP.md"])
        self.assertNotIn("diagnostics slice for each mapped role", docs["DASHBOARD_SETUP.md"])
        ...
        self.assertIn("UI_IMPLEMENTATION_MAP.md — DEPRECATED", ui_implementation_map)
        self.assertIn("no longer a project source of truth", ui_implementation_map)
        self.assertIn("docs/ACTIVE_USER_REQUESTS.md", ui_implementation_map)
