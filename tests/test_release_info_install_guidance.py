...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_build_release_info_uses_current_release_changelog_entry_with_release_date(self) -> None:
        ...
        self.assertEqual(info["released_on"], "2026-07-15")
        ...
        self.assertIn("Managed Devices unmanaged-candidate promotion", info["changes_preview"])
        self.assertIn("Fleet List", info["changes_preview"])
        self.assertIn("numeric managed-device priority", info["changes_preview"])
        self.assertIn("zero_net_export.promote_managed_device", info["changes_preview"])
    ...
    def test_current_candidate_changelog_tracks_corrective_release_scope(self) -> None:
        ...
        self.assertIn("Managed Devices unmanaged-candidate promotion", current_highlights)
        self.assertIn("Fleet List", current_highlights)
        self.assertIn("numeric managed-device priority", current_highlights)
        self.assertIn("zero_net_export.promote_managed_device", current_highlights)
