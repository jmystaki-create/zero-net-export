...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_ui_implementation_map_detailed_work_uses_0191_integration_page_scope(self) -> None:
        ...
        self.assertIn("helper now resolves the current component boundary to `0b4f420` / manifest `0.1.94`", detailed_map)
        self.assertIn("ask James directly whether current `0b4f420` / manifest `0.1.94` replaces the documented `0.1.91` target", detailed_map)
        self.assertIn("`7217f3b`, the post-tag `c4802a3` component boundary, the unapproved `db5c246` / `v0.1.92` freeze, the unapproved `026f189` / `v0.1.93` freeze, and the newer `4c0d071` / `v0.1.94` freeze, and the post-freeze `0b4f420` component boundary are not deploy approval", detailed_map)
    ...
    def test_0191_release_plan_and_bug_tracker_make_human_boundary_explicit(self) -> None:
        ...
        self.assertIn("until James explicitly decides whether current `0b4f420` / manifest `0.1.94` replaces the documented `0.1.91` release target", release_plan)
        self.assertIn("do not treat the `v0.1.91` freeze/tag, the unapproved `v0.1.92` freeze/tag, the unapproved `v0.1.93` freeze/tag, or the unapproved `v0.1.94` freeze/tag or post-freeze `0b4f420` component boundary", zne_429)
        self.assertIn("Ask James directly whether current `0b4f420` / manifest `0.1.94` should replace", zne_429)
        ...
        self.assertIn("now resolves as manifest `0.1.94` / preferred validation commit `0b4f420`", zne_429)
