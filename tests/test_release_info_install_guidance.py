...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_build_release_info_uses_current_release_changelog_entry_with_release_date(self) -> None:
        ...
        self.assertEqual(info["released_on"], "2026-08-02")
        ...
        self.assertIn("Overview", info["changes_preview"])
        self.assertIn("operator-first command center", info["changes_preview"])
        self.assertIn("live power snapshot", info["changes_preview"])
        self.assertIn("state-aware executor controls", info["changes_preview"])
    ...
    def test_current_candidate_changelog_tracks_corrective_release_scope(self) -> None:
        ...
        self.assertIn("Overview", current_highlights)
        self.assertIn("operator-first command center", current_highlights)
        self.assertIn("promoted managed-device review action", current_highlights)
        self.assertIn("state-aware executor controls", current_highlights)
