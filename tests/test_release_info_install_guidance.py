...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_ui_implementation_map_detailed_work_uses_0191_integration_page_scope(self) -> None:
        ...
        self.assertIn("helper now resolves the frozen candidate to `026f189` / `v0.1.93`", detailed_map)
        self.assertIn("ask James directly whether `v0.1.93` replaces the documented `0.1.91` target", detailed_map)
        self.assertIn("`7217f3b`, the post-tag `c4802a3` component boundary, the unapproved `db5c246` / `v0.1.92` freeze, and the newer `026f189` / `v0.1.93` freeze are not deploy approval", detailed_map)
    ...
    def test_0191_release_plan_and_bug_tracker_make_human_boundary_explicit(self) -> None:
        ...
        self.assertIn("later `v0.1.93` at `026f189`", release_plan)
        self.assertIn("Do not deploy, restart, fingerprint-validate, or call successful either the old `7217f3b` tag", release_plan)
        self.assertIn("until James explicitly decides whether `v0.1.93` replaces the documented `0.1.91` release target", release_plan)
        self.assertIn("do not treat the `v0.1.91` freeze/tag, the unapproved `v0.1.92` freeze/tag, or the unapproved `v0.1.93` freeze/tag", zne_429)
        self.assertIn("Ask James directly whether `026f189` / `v0.1.93` should replace", zne_429)
        ...
        self.assertIn("now resolves as manifest `0.1.93` / preferred validation commit `026f189`", zne_429)
