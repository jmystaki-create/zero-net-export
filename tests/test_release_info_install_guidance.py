...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_install_repair_step_for_manifest_mismatch_names_resolved_target_and_commands(self) -> None:
        ...
        self.assertIn("Ask James directly whether the current helper-resolved manifest 0.1.94 component boundary replaces the documented 0.1.91 release target", message)
        self.assertIn("release execution returns to the approved 0.1.91 boundary", message)
        self.assertIn("then ask James to accept or reject the managed-only native device-list representation with a visible settings affordance and no peer Un Managed rows", message)
        self.assertIn("ask James to approve exact release/deploy/restart validation for that accepted target", message)
    ...
    def test_install_repair_step_for_manifest_error_mentions_same_live_install_path(self) -> None:
        ...
        self.assertIn("ask james directly whether the current helper-resolved manifest 0.1.94 component boundary replaces the documented 0.1.91 release target", message.lower())
        self.assertIn("release execution returns to the approved 0.1.91 boundary", message)
        self.assertIn("then ask James to accept or reject the managed-only native device-list representation with a visible settings affordance and no peer Un Managed rows", message)
        self.assertIn("ask James to approve exact release/deploy/restart validation for that accepted target", message)
        self.assertIn("/srv/homeassistant/config/custom_components", message)
