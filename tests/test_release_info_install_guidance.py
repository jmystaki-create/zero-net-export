...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_ui_implementation_map_detailed_work_uses_0191_integration_page_scope(self) -> None:
        ...
        self.assertIn("helper now resolves the frozen candidate to `db5c246` / `v0.1.92`", detailed_map)
        self.assertIn("ask James directly whether `v0.1.92` replaces the documented `0.1.91` target", detailed_map)
        self.assertIn("`7217f3b`, the post-tag `c4802a3` component boundary, and the newer `db5c246` / `v0.1.92` freeze are not deploy approval", detailed_map)
    ...
    def test_0191_release_plan_and_bug_tracker_make_human_boundary_explicit(self) -> None:
        ...
        self.assertIn("repo later froze `v0.1.92` at `db5c246`", release_plan)
        self.assertIn("Do not deploy, restart, fingerprint-validate, or call successful either the old `7217f3b` tag", release_plan)
        self.assertIn("until James explicitly decides whether `v0.1.92` replaces the documented `0.1.91` release target", release_plan)
        self.assertIn("do not treat either the `v0.1.91` freeze/tag or the unapproved `v0.1.92` freeze/tag", zne_429)
        self.assertIn("Ask James directly whether `db5c246` / `v0.1.92` should replace", zne_429)
        ...
        self.assertIn("helper now resolves the install candidate to manifest `0.1.92`", zne_429)
