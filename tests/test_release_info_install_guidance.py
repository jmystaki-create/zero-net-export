...
class ReleaseInfoInstallGuidanceTests(unittest.TestCase):
    ...
    def test_build_release_info_uses_current_release_changelog_entry_with_release_date(self) -> None:
        ...
        self.assertEqual(info["released_on"], "2026-06-30")
        ...
        self.assertIn("sidebar app workflow", info["changes_preview"])
        self.assertIn("Managed Devices", info["changes_preview"])
        self.assertIn("REMOVE FROM ZNE", info["changes_preview"])
    ...
    def test_current_candidate_changelog_tracks_application_release_scope(self) -> None:
        ...
        self.assertIn("sidebar app workflow slice", current_highlights)
        self.assertIn("Overview, Sources, Managed Devices, Controls", current_highlights)
        self.assertIn("REMOVE FROM ZNE", current_highlights)
        self.assertIn("controller enabled, mode", current_highlights)
        self.assertIn("GitHub", current_highlights)
        self.assertIn("HACS", current_highlights)
