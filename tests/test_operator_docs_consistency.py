...
class OperatorDocsConsistencyTests(unittest.TestCase):
    ...
    def test_readme_tracks_current_091_release_target_hold(self) -> None:
        ...
        first_gate = status.index("whether the current helper-resolved manifest `0.1.94` component boundary replaces")
        native_gate = status.index("then accepts the closest native child-device representation")
        release_gate = status.index("then gives exact release/deploy/restart approval")
        self.assertLess(first_gate, native_gate)
    ...
    def test_readme_manual_deploy_guidance_keeps_approval_order(self) -> None:
        ...
        self.assertIn("only after James has made the current release-target decision, accepted the closest native child-device representation, and approved exact release/deploy/restart", content)
