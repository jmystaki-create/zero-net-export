...
class ButtonEntityCategoryTests(unittest.TestCase):
    ...
    def test_blocker_handoff_after_repair_keeps_empty_fleet_manual_in_managed_devices_workspace_when_no_candidates_exist(self) -> None:
        ...
        self.assertIn(
            "- After repair: Open devices path to review the Managed Devices workspace, then add the first fixed or variable load manually when no surfaced unmanaged candidate is available right now.",
            lines,
        )
        self.assertNotIn(
