...
def _load_button_module(notification_calls: list[dict] | None = None):
    ...
    native_support_module.build_native_command_center_summary = lambda coordinator: {
        ...
        "detailed_management_summary": "Use the device page for secondary per-device ⚙ Settings review/audit.",
...
class ButtonEntityCategoryTests(unittest.TestCase):
    ...
    def test_workspace_handoff_returns_promotion_steps_when_managed_devices_is_the_active_section(self) -> None:
        ...
        self.assertIn("- Use detailed device path afterward only if you need secondary per-device ⚙ Settings review/audit.", handoff)
    ...
    def test_fleet_console_button_renders_managed_vs_unmanaged_workspace(self) -> None:
        ...
        self.assertEqual(button._attr_name, "⚙ Settings — Open Managed Devices workspace")
        self.assertEqual(button._attr_icon, "mdi:cog-outline")
        self.assertEqual(button._attr_device_info["name"], "Test Entry")
        self.assertEqual(button._attr_device_info["identifiers"], {("zero_net_export", "entry-1")})
        ...
        self.assertIn(
            "Device-page boundary: Make promotion, enablement, removal, and other fleet edits in devices path; use the device page only for secondary ⚙ Settings review/audit and handoff.",
            message,
        )
        ...
        self.assertNotIn("Managed devices (top section):", message)
        ...
        self.assertNotIn("Managed devices needing attention first:", message)
        ...
        self.assertIn(
            "- Ready-next unmanaged usefulness: likely useful: Switch entities are usually likely fixed-load candidates when they control a real appliance or relay.",
            message,
        )
        ...
        self.assertIn("- Use detailed device path only for secondary per-device ⚙ Settings review/audit after the main fleet step is clear.", message)
        self.assertIn(
            "- Make promotion, enablement, removal, and other fleet edits in devices path; use the device page only for secondary ⚙ Settings review/audit and handoff.",
            message,
        )
        self.assertNotIn("secondary per-device review/audit", message)
        self.assertNotIn("secondary review/audit and handoff", message)
    ...
    def test_managed_device_review_button_renders_runtime_fleet_summary(self) -> None:
        ...
        button = button_module.ZeroNetExportShowManagedDeviceReviewButton(coordinator)
        self.assertEqual(button._attr_name, "⚙ Settings — Open Managed Devices review")
        self.assertEqual(button._attr_icon, "mdi:cog-outline")
        self.assertEqual(button._attr_device_info["name"], "Test Entry")
        self.assertEqual(button._attr_device_info["identifiers"], {("zero_net_export", "entry-1")})
        button.hass = SimpleNamespace(
        ...
        self.assertIn("Currently surfaced unmanaged candidates:", message)
        self.assertIn("- Hot water (fixed) | likely useful | key warning: No immediate warnings", message)
        self.assertIn("Secondary ⚙ Settings device-page audit path: detailed device path", message)
        self.assertIn(
            "Device-page boundary: Make promotion, enablement, removal, and other fleet edits in devices path; use the device page only for secondary ⚙ Settings review/audit and handoff.",
            message,
        )
    ...
    def test_managed_device_review_button_exposes_unmanaged_candidate_attributes(self) -> None:
        ...
        self.assertEqual(attrs["managed_snapshot"], "1 managed | 1 enabled | 1 usable | active load 1180 W | 1 active managed device | active device Pool pump (action turn_on | active 1180 W) | 1 managed device needs attention | attention first Pool pump | 1 planned action | plan Pool pump")
    ...
    def test_blocker_handoff_after_repair_keeps_existing_fleet_on_managed_devices_workspace_when_no_candidates_exist(self) -> None:
        ...
        self.assertIn(
            "- After repair: Open devices path and use the Managed Devices workspace to edit device settings or stage enablement changes before changing Controls settings or opening Diagnostics.",
            lines,
        )
    ...
    def test_empty_planned_action_does_not_create_a_fake_active_plan_snapshot(self) -> None:
        ...
        self.assertEqual(review_attrs["managed_snapshot"], "1 managed | 1 enabled | 1 usable | 0 planned actions")
    ...
    def test_managed_snapshot_names_first_attention_device_without_blocked_or_planned_rows(self) -> None:
        ...
        self.assertEqual(
            summary,
            "1 managed | 1 enabled | 1 usable | 1 managed device needs attention | attention first Pool pump | 1 fixed managed | 1200 W nominal | 0 planned actions",
        )
    ...
    def test_managed_snapshot_prefers_attention_first_order_over_config_insertion_order(self) -> None:
        ...
        self.assertEqual(
            summary,
            "2 managed | 2 enabled | 1 usable | 2 managed devices need attention | attention first Pool pump | 1 fixed managed | 1 variable managed | 0 W nominal | blocked Pool pump | 2 planned actions | plan Pool pump",
        )
    ...
    def test_managed_snapshot_keeps_active_device_when_runtime_watts_are_missing(self) -> None:
        ...
        self.assertEqual(
            summary,
            "2 managed | 2 enabled | 2 usable | 1 active managed device | active device Heated floor (variable | active) | 1 managed device needs attention | attention first Pool pump | 1 fixed managed | 1 variable managed | 3385 W nominal | 1 planned action | plan Pool pump",
        )
    ...
    def test_managed_device_detail_button_renders_per_device_review(self) -> None:
        ...
        self.assertNotIn("Test Entry: managed devices review", notification_calls[0]["kwargs"]["title"])
        message = notification_calls[0]["args"][1]
        self.assertIn("Zero Net Export ⚙ Settings — managed-device detail review", message)
        self.assertIn("Managed Devices workspace: devices path", message)
        ...
        self.assertNotIn("Managed devices workspace context:", message)
    ...
    def test_managed_device_review_attributes_treat_blocked_plans_as_blocked_activity(self) -> None:
        ...
        self.assertEqual(attrs["managed_snapshot"], "1 managed | 1 enabled | 1 usable | 1 managed device needs attention | attention first Pool pump | 1 fixed managed | 0 W nominal | blocked Pool pump | 1 planned action | plan Pool pump")
    ...
    def test_setup_checklist_button_keeps_notification_checklist_focused(self) -> None:
        ...
        self.assertIn("- [x] Source roles: Solar power and grid export mapped.", message)
        ...
        self.assertNotIn("- [x] Source map:", message)
    ...
    def test_command_center_guide_button_uses_shared_full_guide_text(self) -> None:
        ...
        self.assertIn("Managed-device audit path: Use the device page for secondary per-device ⚙ Settings review/audit.", message)
