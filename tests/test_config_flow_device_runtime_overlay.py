...
class ConfigFlowDeviceRuntimeOverlayTests(unittest.TestCase):
    ...
    def test_device_pick_candidate_forms_surface_blocker_and_workspace_snapshots(self) -> None:
        ...
        self.assertEqual(
            ...
            "3 candidates | 2 fixed candidates | 1 variable candidate | fixed backlog 2 review | variable backlog 1 review | 3 need review | 2 fixed review | 1 variable review | review AC Outlet 2 | likely useful | key warning: No immediate warnings",
        )
    ...
    def test_device_actions_surface_managed_and_unmanaged_counts_in_selector_labels(self) -> None:
        ...
        flow._load_devices = lambda: (
            [
                ...
                {"key": "ev", "name": "EV charger", "kind": module.DEVICE_KIND_VARIABLE, "usable": True},
            ],
            [],
        )
        flow._device_candidates = lambda: [
        ...
        original_assess_candidate = module.assess_candidate
        original_candidate_needs_review = module.candidate_needs_review
        module.assess_candidate = lambda candidate: {"needs_review": candidate.get("name") == "Virtual load"}
        module.candidate_needs_review = lambda summary: bool(summary.get("needs_review"))
        ...
        self.assertEqual(
            options[1]["label"],
            "Promote variable-load candidate / 1 variable candidate surfaced / 1 candidate ready to promote",
        )
        self.assertEqual(
            options[2]["label"],
    ...
    def test_device_actions_surface_ready_to_promote_counts_when_review_backlog_is_clear(self) -> None:
        ...
        original_assess_candidate = module.assess_candidate
        original_candidate_needs_review = module.candidate_needs_review
        module.assess_candidate = lambda candidate: {"needs_review": False}
        module.candidate_needs_review = lambda summary: bool(summary.get("needs_review"))
        ...
        self.assertEqual(
            options[0]["label"],
            "Promote fixed-load candidate / 1 fixed candidate surfaced / 1 candidate ready to promote",
        )
        self.assertEqual(
            options[1]["label"],
            "Promote variable-load candidate / 1 variable candidate surfaced / 1 candidate ready to promote",
    ...
    def test_detailed_management_summary_falls_back_to_secondary_review_path_wording(self) -> None:
        ...
        self.assertEqual(
            ...
            f"Use {module.DETAILED_MANAGEMENT_PATH} as the secondary ⚙ Settings device-page review/audit path for managed-device audit detail.",
        )
    ...
    def test_managed_snapshot_treats_non_executable_plans_as_blocked_before_usable_flips_false(self) -> None:
        ...
        self.assertEqual(
            flow._managed_snapshot_text(devices),
            "2 managed | 2 enabled | 2 usable | 2 managed devices need attention | attention first Pool pump (fixed | blocked) | 1 fixed managed | 1 variable managed | 0 W nominal | blocked Pool pump (fixed | blocked) | 2 planned actions | plan Pool pump (fixed | blocked)",
        )
        self.assertEqual(
    ...
    def test_managed_snapshot_and_fleet_summary_surface_active_managed_runtime(self) -> None:
        ...
        self.assertEqual(
            flow._managed_snapshot_text(devices),
            "2 managed | 2 enabled | 2 usable | active load 3385 W | 2 active managed devices | 2 managed devices need attention | attention first EV charger (variable | action set_power | active 2200 W) | 1 fixed managed | 1 variable managed | 8200 W nominal | 2 planned actions | plan EV charger (variable | action set_power | active 2200 W)",
        )
        self.assertEqual(
            flow._fleet_summary_lines(devices)[0],
            "- Fleet summary: 2 managed devices, 2 managed devices need attention, attention first EV charger (variable | action set_power | active 2200 W), 2 planned actions, plan EV charger (variable | action set_power | active 2200 W), active load 3385 W, 2 active managed devices, 2 enabled, 2 usable, 0 blocked, 1 fixed, 1 variable, 8200 W nominal controllable power",
        )
    ...
    def test_managed_snapshot_keeps_active_device_visible_when_attention_and_runtime_differ(self) -> None:
        ...
        self.assertEqual(
            flow._managed_snapshot_text(devices),
            "2 managed | 2 enabled | 2 usable | active load 920 W | 1 active managed device | active device Heated floor (variable | active 920 W) | 1 managed device needs attention | attention first Pool pump (fixed | action turn_on) | 1 fixed managed | 1 variable managed | 3385 W nominal | 1 planned action",
        )
        self.assertEqual(
            flow._fleet_summary_lines(devices)[0],
            "- Fleet summary: 2 managed devices, 1 managed device needs attention, attention first Pool pump (fixed | action turn_on), 1 planned action, active load 920 W, 1 active managed device, active device Heated floor (variable | active 920 W), 2 enabled, 2 usable, 0 blocked, 1 fixed, 1 variable, 3385 W nominal controllable power",
        )
    ...
    def test_managed_snapshot_names_active_device_when_fleet_is_healthy(self) -> None:
        ...
        self.assertEqual(
            flow._managed_snapshot_text(devices),
            "2 managed | 2 enabled | 2 usable | active load 2105 W | 2 active managed devices | active device Heated floor (variable | active 920 W) | 1 fixed managed | 1 variable managed | 3385 W nominal",
        )
        self.assertEqual(
            flow._fleet_summary_lines(devices)[0],
            "- Fleet summary: 2 managed devices, active load 2105 W, 2 active managed devices, active device Heated floor (variable | active 920 W), 2 enabled, 2 usable, 0 blocked, 1 fixed, 1 variable, 3385 W nominal controllable power",
        )

    def test_managed_snapshot_keeps_active_device_visible_without_runtime_watts(self) -> None:
        module = _load_config_flow_module()
        flow = module.ZeroNetExportOptionsFlow(SimpleNamespace(entry_id="entry-1", options={}, data={}))
        ...
        self.assertEqual(
            flow._managed_snapshot_text(devices),
            "2 managed | 2 enabled | 2 usable | 1 active managed device | active device Heated floor (variable | active) | 1 managed device needs attention | attention first Pool pump (fixed | action turn_on) | 1 fixed managed | 1 variable managed | 3385 W nominal | 1 planned action",
        )
        self.assertEqual(
            flow._fleet_summary_lines(devices)[0],
            "- Fleet summary: 2 managed devices, 1 managed device needs attention, attention first Pool pump (fixed | action turn_on), 1 planned action, 1 active managed device, active device Heated floor (variable | active), 2 enabled, 2 usable, 0 blocked, 1 fixed, 1 variable, 3385 W nominal controllable power",
        )
    ...
    def test_existing_fleet_forms_keep_managed_and_unmanaged_snapshots_visible(self) -> None:
        ...
        for result in (bulk_enable, edit_pick, remove):
            ...
            self.assertEqual(
                ...
                "2 candidates | 2 fixed candidates | fixed backlog 2 review | 2 need review | 2 fixed review | review AC Outlet 2 | likely useful | key warning: No immediate warnings",
            )
    ...
    def test_promotion_forms_keep_managed_and_unmanaged_context_visible(self) -> None:
        ...
        for result in (vetting, template, add):
            ...
            self.assertEqual(
                result["description_placeholders"]["managed_snapshot"],
                "2 managed | 2 enabled | 1 usable | 2 managed devices need attention | attention first EV charger (variable | not usable) | 1 fixed managed | 1 variable managed | 0 W nominal | blocked EV charger (variable | not usable) | 1 planned action | plan Pool pump (fixed | action on)",
            )
            self.assertEqual(
    ...
    def test_healthy_source_next_step_uses_workspace_first_candidate_handoff_when_candidates_remain(self) -> None:
        ...
        self.assertEqual(
            placeholders["source_next_step"],
            f"Open {module.DEVICES_CONFIGURE_PATH} to continue in the Managed Devices workspace, review unmanaged candidate: Hot water (fixed) | likely useful | key warning: No immediate warnings.",
        )
    ...
    def test_healthy_source_next_step_keeps_ready_next_candidate_visible_after_review_first(self) -> None:
        ...
        self.assertEqual(
            placeholders["source_next_step"],
            f"Open {module.DEVICES_CONFIGURE_PATH} to continue in the Managed Devices workspace, review unmanaged candidate: Virtual load (fixed) | likely useful, then promote ready unmanaged candidate: Dishwasher Power (fixed) | likely useful.",
        )
    ...
    def test_build_device_action_feedback_for_promotion_uses_native_paths(self) -> None:
        ...
        self.assertIn("2 candidates | 1 fixed candidate | 1 variable candidate | fixed backlog 1 review | variable backlog 1 review | 2 need review | 1 fixed review | 1 variable review | review AC Outlet 2 | likely useful | key warning: No immediate warnings", feedback["message"])
        ...
        self.assertIn("Managed Devices workspace: devices path", feedback["message"])
        self.assertNotIn("Managed Devices workspace in Configure", feedback["message"])
        self.assertIn(
            "Secondary ⚙ Settings device-page review/audit path, after the main fleet step is clear: detailed device path",
            feedback["message"],
        )
        self.assertIn(
            "Next step: reopen devices path to continue in the Managed Devices workspace, then review unmanaged candidate: AC Outlet 2 (fixed) | likely useful | key warning: No immediate warnings.",
            feedback["message"],
        )
    ...
    def test_unmanaged_snapshot_keeps_single_kind_review_backlog_visible(self) -> None:
        module = _load_config_flow_module()
        module.assess_candidate = lambda candidate: {
            "confidence": "low",
            "summary": "Review before promotion.",
            "warnings": ["Needs review first."],
        }
        module.build_candidate_review_hint = lambda candidate, include_warning=True, max_warning_chars=56, **kwargs: (
            "review first | warn Needs review first." if include_warning else "review first"
        )
        ...
        self.assertIn("2 fixed candidates", summary)
        self.assertIn("fixed backlog 2 review", summary)
    ...
    def test_build_device_action_feedback_remove_prefers_review_first_candidate(self) -> None:
        ...
        assert feedback is not None
        self.assertIn(
            "Next step: reopen devices path to continue in the Managed Devices workspace, then review unmanaged candidate: Virtual load (fixed) | review first | key warning: Helper-backed load needs review, then promote ready unmanaged candidate: Hot water (fixed) | likely useful | key warning: No immediate warnings.",
            feedback["message"],
        )
        self.assertNotIn("remaining fleet", feedback["message"])
    ...
    def test_device_next_step_keeps_managed_devices_workspace_wording_when_candidates_remain(self) -> None:
        ...
        self.assertEqual(
            next_step,
            "Open devices path to continue in the Managed Devices workspace, review unmanaged candidate: Hot water (fixed) | likely useful | key warning: No immediate warnings.",
        )
    ...
    def test_device_next_step_keeps_ready_next_candidate_visible_after_review_first(self) -> None:
        ...
        self.assertEqual(
            next_step,
            "Open devices path to continue in the Managed Devices workspace, review unmanaged candidate: Virtual load (fixed) | likely useful, then promote ready unmanaged candidate: Dishwasher Power (fixed) | likely useful.",
        )
