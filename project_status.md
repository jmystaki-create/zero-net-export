...
next_action: stop spending runs on stale-hash cleanup; the current component-changing candidate is exact build `840695e`, so first judge whether any remaining repo-side promotion-flow wording drift still needs fixing before freeze, and only then ask James directly for explicit deploy/restart approval on that build

# Current blocker or none
blocker: unchanged live exact-build drift, confirmed again over the documented HA SSH path; the live install is still a divergent `0.1.87` build, and release approval should only be considered against the current component-changing candidate `840695e` rather than any older stale target

# Exact user action needed or none
user_action: none until the `840695e` cut line is judged coherent enough for formal deploy/restart approval
...
last_modified: 2026-04-19 21:58
