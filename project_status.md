...
next_action: ask James directly whether to proceed with the formal `0.1.86` release-reconciliation flow now, then deploy one exact repo build that resolves the remaining live `config_flow.py`, `native_support.py`, and `sensor.py` drift before any further UI claims

# Current blocker or none
blocker: the live Home Assistant install is still not fingerprint-aligned because `config_flow.py`, `native_support.py`, and `sensor.py` do not match repo HEAD, and the real next boundary is explicit release approval rather than more implied deploy/restart guidance
...
last_modified: 2026-04-16 18:53
