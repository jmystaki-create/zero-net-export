...
next_action: ask James directly to approve the formal `0.1.85` release flow now, then redeploy the exact repo candidate, clear the remaining `entity.py` fingerprint drift, and continue live native-UI validation

# Current blocker or none
blocker: formal release approval is still required before redeploy/restart work, and the installed component still has an older `entity.py` than repo HEAD
