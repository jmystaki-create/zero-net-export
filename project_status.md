...
next_action: stop treating the release/fingerprint boundary as the default every-run headline; either name one fresh concrete remaining A-D/F `0.1.87` repo-side gap first, or, if no such gap remains, ask James once for deploy/restart approval against the helper-resolved exact build current at that time
