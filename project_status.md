...
next_action: ask James directly for explicit deploy/restart approval for exact build `21049b5`; the documented HA SSH fingerprint check is still unchanged, and repeating more approval-target bookkeeping would be churn

# Current blocker or none
blocker: unchanged live exact-build drift, confirmed again over the documented HA SSH path; the live install is still a divergent `0.1.87` build with the same eleven-file mismatch against the current component-changing candidate `21049b5`

# Exact user action needed or none
user_action: James needs to approve deploy/restart of exact build `21049b5`
...
last_modified: 2026-04-19 22:57
