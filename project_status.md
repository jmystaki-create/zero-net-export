...
next_action: stop spending runs on stale-hash cleanup; the repo candidate is now exact build `bade75f`, so either ask James directly for explicit deploy/restart approval on that build or keep moving the next mapped non-live stage only if a fresh repo-side drift appears

# Current blocker or none
blocker: unchanged live exact-build drift, confirmed again over the documented HA SSH path; the live install is still a divergent `0.1.87` build, and the real boundary is explicit deploy/restart approval for exact build `bade75f`, not another release-target wording refresh

# Exact user action needed or none
user_action: James must explicitly approve deploy/restart of exact build `bade75f` before formal release execution
