...
- **repo fix:** earlier work updated `custom_components/zero_net_export/sensor.py` so every configured load now exposes a primary `managed summary` sensor on the Zero Net Export device page, while deeper per-device runtime history sensors such as last-requested power, last-applied power, durations, timestamps, and last-action detail move into diagnostic categorization. This same bug fix path also renamed the per-device review buttons in `custom_components/zero_net_export/button.py` to explicit `{device} managed review` labels so the device-page managed-device actions group more clearly. This run tightens the device-page surface again by moving per-device status, guard status, planned action, and planned power delta sensors into `EntityCategory.DIAGNOSTIC`, leaving the primary device page focused on each load's managed summary plus live power/target entities.
- **validation status:** repo-side fix advanced again in this run and is now covered by `python3 -m unittest tests.test_sensor_entity_categories tests.test_button_entity_categories -q` plus `python3 -m unittest discover -s tests -q`. Live Home Assistant validation is still pending on the next exact-build redeploy.
- **next action:** redeploy the current repo candidate, then confirm the Zero Net Export device page now centers each configured load on the managed summary row plus live power/target entities while the secondary status/guard/plan detail sits under diagnostics instead of crowding the primary device surface
