...
## ZNE-014 — Post-install log review confirms the live 0.1.85 install is still not fully healthy
- **status:** `fixed_pending_validation`
- **severity:** `high`
- **area:** `runtime`
- **where seen:** live Home Assistant `0.1.85` after user install and manual log review request
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The live system still appears materially healthier than before, but this run's documented HA log review still shows the known blocking-I/O traceback from the older live `entity.py`, and fingerprint validation now confirms the installed component is still behind the current repo candidate in `candidate_utils.py`, `entity.py`, `sensor.py`, `strings.json`, and `translations/en.json`, so install health is not fully back until the exact repo candidate is redeployed.
...
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again, `ha core logs -n 120 | tail -n 120` still shows the older live `entity.py` traceback ending at `_device_surface_version()` and `manifest_path.read_text(...)` at `2026-04-16 10:03`, and `scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `ca7e878` / component commit `23d475b` still reports `overall_match=false` with shipped-file mismatches in `candidate_utils.py` (`755215fedf19` live vs `bbffc1f2d809` repo), `entity.py` (`2dffb9a4ef0c` live vs `9b087d27adff` repo), `sensor.py` (`c7218b24e49b` live vs `6c87c053febb` repo), `strings.json` (`6bdcb2e66f13` live vs `0eb43eca5ca5` repo), and `translations/en.json` (`6bdcb2e66f13` live vs `0eb43eca5ca5` repo), so the install-health blocker is now primarily exact-build drift rather than a new runtime traceback.
...
- **validation status:** materially improved again in this run. The old setup failure no longer looks like the immediate blocker, but this umbrella bug should stay open until the exact repo candidate is redeployed and a post-restart log review shows the live install is both healthy and fingerprint-aligned without the known `entity.py` blocking-I/O warnings or the remaining `candidate_utils.py` / `sensor.py` / shipped-copy drift.
- **next action:** treat exact-build redeploy and post-restart validation under ZNE-022 as the real remaining work, then close this umbrella bug once the live install matches the repo candidate and the stale `entity.py` blocking-I/O warning is gone
...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate in this run. After refreshing the repo working version back to the source-of-truth `0.1.85` release line and rechecking live HA, the install had previously narrowed to one exact-build mismatch in `entity.py`; after later repo-side candidate-shortlist, managed-summary, and native-copy fixes landed, the current live install is now behind repo HEAD in five shipped files: `candidate_utils.py`, `entity.py`, `sensor.py`, `strings.json`, and `translations/en.json`.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo metadata fix, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reported live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and mismatches in `entity.py` plus `manifest.json`. A later recheck narrowed the live install drift to `entity.py` only (`/homeassistant/custom_components/zero_net_export/entity.py` `sha256_12=2dffb9a4ef0c`, repo `sha256_12=9b087d27adff`) while HA logs still showed the known blocking-I/O warnings from that older file. In this run, a fresh fingerprint rerun against repo commit `ca7e878` with expected component commit `23d475b` reports live manifest `0.1.85`, `overall_match=false`, and shipped-file mismatches in `candidate_utils.py` (`755215fedf19` live vs `bbffc1f2d809` repo), `entity.py` (`2dffb9a4ef0c` live vs `9b087d27adff` repo), `sensor.py` (`c7218b24e49b` live vs `6c87c053febb` repo), `strings.json` (`6bdcb2e66f13` live vs `0eb43eca5ca5` repo), and `translations/en.json` (`6bdcb2e66f13` live vs `0eb43eca5ca5` repo), which is expected until the current repo candidate is redeployed end to end.
- **suspected cause:** repo HEAD and the live Home Assistant component tree have mostly converged again, but the installed package still carries older Python and shipped-copy files than repo HEAD, including stale native wording in the live `strings.json` and `translations/en.json`.
- **repo fix:** `afbbd71`, `cf46eac`, `b9766aa`, `f45aef9` — widen exact-build fingerprint coverage beyond the old hand-maintained subset, then derive tracked files from the actual shipped component tree in `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py`, skipping only `__pycache__`; tests now assert coverage for active shipped files like `button.py`, `candidate_utils.py`, `binary_sensor.py`, `repairs.py`, and `select.py` so future UI/runtime files cannot silently fall outside exact-build validation, and this run's bug-state refresh records the current exact mismatch set and target commit from the live fingerprint output.
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The current exact-build mismatch set is now the still-older live `candidate_utils.py`, `entity.py`, `sensor.py`, `strings.json`, and `translations/en.json`, and the matching HA log warning on the older live `entity.py` confirms the repo-side release blocker is deployment rather than another hidden repo regression. The real boundary is now explicit release approval to redeploy the shipped `0.1.85` candidate end to end and rerun fingerprint validation until it returns `overall_match=true`.
- **next action:** ask James directly to approve the full end-to-end release flow for the exact current shipped-component `0.1.85` candidate, redeploy it, rerun fingerprint validation until it reports `overall_match=true`, then restart Home Assistant and continue Configure/native-UI validation on the exact installed candidate
