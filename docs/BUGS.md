...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` now reports repo commit `6f9cff2`, live manifest `0.1.83`, and `overall_match=false`, with mismatches at `button.py`, `candidate_utils.py`, `config_flow.py`, `native_support.py`, `release_info.py`, `strings.json`, and `translations/en.json`. This is no longer just abstract drift: the same SSH session's `ha core logs -n 400 | grep -i zero_net_export` still shows the old blocking `release_info.py` warning path at `2026-04-16 00:09:48`, which means current live validation is still being contaminated by stale installed code.
...
- **repo fix:** `afbbd71`, `cf46eac`, `b9766aa` — widen exact-build fingerprint coverage beyond the old hand-maintained subset, then derive tracked files from the actual shipped component tree in `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py`, skipping only `__pycache__`; tests now assert coverage for active shipped files like `button.py`, `candidate_utils.py`, `binary_sensor.py`, `repairs.py`, and `select.py` so future UI/runtime files cannot silently fall outside exact-build validation.
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. Repo state itself is not the blocker: `git status --short` is clean and `git log --oneline -n 1` shows `6f9cff2` at `HEAD`, so the real boundary is still explicit release approval to redeploy the exact current candidate and rerun fingerprint validation until it returns `overall_match=true`.
- **next action:** ask James directly for release approval to redeploy the exact current repo build from commit `6f9cff2`, rerun fingerprint validation until it reports `overall_match=true`, then restart Home Assistant and continue Configure/native-UI validation on the exact installed candidate
