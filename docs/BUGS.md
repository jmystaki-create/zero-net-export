...
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The live system remains materially closer to the repo candidate than before, but it is still not in a trustworthy release state because the exact-build fingerprint is no longer down to a version-line mismatch alone: the live install is still advertising `0.1.86`, and it is also missing the current repo `sensor.py` fleet-workspace update from `b7efb75`.
- **expected behavior:** after install, Zero Net Export should load cleanly, allow Add Integration, avoid integration-specific live errors in Home Assistant logs, and match the exact repo candidate under review, including the shipped manifest version.
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again, and `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `b7efb75` reports `overall_match=false` with two remaining shipped-file mismatches: live manifest `0.1.86` (`sha256_12=62bcb7ffe76b`) versus repo manifest `0.1.85` (`sha256_12=14a428ae3934`), and live `sensor.py` (`sha256_12=6c87c053febb`, 37812 bytes) versus repo `sensor.py` (`sha256_12=cef79b3dd541`, 39528 bytes).
- **suspected cause:** the original install-health failure overlapped ZNE-002 and ZNE-011; the remaining live-health concern now overlaps ZNE-022 because the installed component payload has effectively converged to the repo candidate while the live manifest/version line was left one step ahead, creating a mixed release state.
- **validation status:** materially improved versus the earlier broken install, but this umbrella bug should stay open. The current blocker set is smaller than before, yet it is not just a manifest/version mismatch anymore because the latest repo `sensor.py` has also not landed in Home Assistant. Keep this open until both mismatches are reconciled under the formal release flow and a post-restart log review confirms the live install is healthy and fingerprint-aligned.
- **next action:** treat the remaining manifest/version mismatch plus undeployed `sensor.py` drift under ZNE-022 as the real release blockers, then close this umbrella bug once the live install matches the intended repo candidate end to end and post-restart health stays clean
...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate in this run, and the blocker set changed again. The live Home Assistant component payload no longer differs only on `manifest.json`; it is also missing the current repo `sensor.py` fleet-workspace update from `b7efb75`. The live install is therefore still advertising `0.1.86` while also lagging one shipped UI/runtime file behind the current `0.1.85` repo candidate.
- **expected behavior:** before trusting further live validation, the Home Assistant install should match the current repo candidate exactly across the full shipped component surface, including the shipped manifest version and release line.
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo metadata fix, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reported live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and mismatches in `entity.py` plus `manifest.json`. A later recheck narrowed the live install drift to `entity.py` only (`/homeassistant/custom_components/zero_net_export/entity.py` `sha256_12=2dffb9a4ef0c`, repo `sha256_12=9b087d27adff`) while HA logs still showed the known blocking-I/O warnings from that older file. In the last bug-state refresh, a fingerprint rerun against repo commit `e19cca9` appeared to isolate the remaining mismatch to `manifest.json`. This run rechecked the documented HA SSH path against current repo commit `b7efb75`, and the fingerprint output now reports two remaining shipped-file mismatches: `manifest.json` (`62bcb7ffe76b` live vs `14a428ae3934` repo) and `sensor.py` (`6c87c053febb` live vs `cef79b3dd541` repo).
- **suspected cause:** the repo working version was corrected back to the source-of-truth `0.1.85` line in `e19cca9`, but the live Home Assistant install still has the earlier `0.1.86` manifest. That leaves the system in a mixed release state where the deployed code payload aligns with the current repo candidate but the visible installed version does not.
- **repo fix:** `afbbd71`, `cf46eac`, `b9766aa`, `f45aef9` — widen exact-build fingerprint coverage beyond the old hand-maintained subset, then derive tracked files from the actual shipped component tree in `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py`, skipping only `__pycache__`; tests now assert coverage for active shipped files like `button.py`, `candidate_utils.py`, `binary_sensor.py`, `repairs.py`, and `select.py` so future UI/runtime files cannot silently fall outside exact-build validation. This run's bug-state refresh keeps the widened fingerprint as the source of truth for the current live mismatch set.
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The current exact-build mismatch set is smaller than the earlier multi-file drift, but it is no longer just the live `manifest.json`: `sensor.py` is also behind repo HEAD. That is still enough to keep release state and live UI/version claims untrustworthy. The real boundary remains explicit release approval to reconcile both mismatches end to end and rerun fingerprint validation until it returns `overall_match=true`.
- **next action:** ask James directly to approve the formal release flow now, then reconcile the live `0.1.86` manifest drift and undeployed `sensor.py` change against the intended `0.1.85` repo candidate, rerun fingerprint validation until it reports `overall_match=true`, restart Home Assistant, and continue Configure/native-UI validation on the exact installed candidate
