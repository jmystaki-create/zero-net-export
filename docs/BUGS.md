...
- **severity:** `medium`
- **area:** `release`
- **where seen:** repo-versus-live fingerprint validation on 2026-04-16 after `b6ddcd2`
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against repo `b6ddcd2`. The live Home Assistant install still matches the earlier deployed `0.1.83` build for most tracked files, but `release_info.py` remains behind the repo candidate, so the exact installed package is no longer the exact code under review.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded in this run. The fingerprint validator reported live manifest `0.1.83`, expected commit `b6ddcd2`, and `overall_match=false`, with the only tracked-file mismatch still at `release_info.py` (`actual sha256_12=71f4ed6d6b00`, `expected sha256_12=5b32d86d6c48`). The same validator output shows `__init__.py`, `config_flow.py`, `coordinator.py`, `manifest.json`, `native_support.py`, `strings.json`, and `translations/en.json` still match the live install.
...
- **validation status:** confirmed live drift again in this run; no deploy/restart performed because release execution still requires explicit approval
- **next action:** ask James directly for release approval to redeploy the exact `b6ddcd2` build, rerun fingerprint validation until it reports `overall_match=true`, then restart Home Assistant and continue Configure/native-UI validation on the exact installed candidate
