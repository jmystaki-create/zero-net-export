...
- **expected behavior:** before trusting further live validation, the Home Assistant install should match the current repo candidate exactly across the tracked files, including the current command-center and managed-device flow copy plus the supporting Python/UI helpers that drive those native surfaces
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded in this run. After the fingerprint coverage fix landed, the validator still reported live manifest `0.1.83` and `overall_match=false`, with tracked-file mismatches at `button.py`, `candidate_utils.py`, `config_flow.py`, `release_info.py`, `strings.json`, and `translations/en.json`. The same validator output still shows `__init__.py`, `coordinator.py`, `manifest.json`, `native_support.py`, `diagnostics.py`, and `sensor.py` matching the live install.
- **suspected cause:** repo HEAD advanced after the earlier live deploy/restart cycle, but the exact current repo candidate has not yet been redeployed to Home Assistant
- **validation status:** confirmed live drift again in this run against the current repo candidate; no deploy/restart performed because release execution still requires explicit approval
