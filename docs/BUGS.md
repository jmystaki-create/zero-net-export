...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. Repo state itself is not the blocker: `git status --short` is clean, repo `HEAD` is now `95c4d80`, and the latest shipped-component candidate remains `6f9cff2`; this run corrected the stale release wording so the approval boundary names the actual redeploy target instead of an older docs-only snapshot. The real boundary is still explicit release approval to redeploy the shipped candidate and rerun fingerprint validation until it returns `overall_match=true`.
