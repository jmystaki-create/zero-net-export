...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. A fresh `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `67ba466` reported `overall_match=false` with `manifest_version_matches=true` and a single remaining mismatch: live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sensor.py` `sha256_12=5c485eb35c56` (40202 bytes). `manifest.json` now matches live at `0.1.86`.
