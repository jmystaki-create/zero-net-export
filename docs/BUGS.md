...
- **evidence:** direct user feedback plus repo/design audit captured in `docs/UI_DESIGN.md` and `docs/UI_IMPLEMENTATION_MAP.md`; repo inspection on 2026-04-15 also confirmed the managed-fleet workspace sensors and fleet-console helper were still marked diagnostic, which buried Managed Devices review under diagnostic-style entity presentation instead of treating it as a first-class fleet workspace; later same-day repo inspection also found the new managed/unmanaged and promotion-flow copy had landed in `custom_components/zero_net_export/strings.json` but not in `custom_components/zero_net_export/translations/en.json`, leaving the shipped English UI text behind the intended native flow wording
- **suspected cause:** implementation is still too scaffold/text-heavy and not yet sufficiently productized in live HA, and translation artifacts can drift when native flow copy changes land without the matching `translations/en.json` update
- **repo fix:** current repo candidate in this run promotes managed-fleet workspace sensors and fleet-console helper out of diagnostics-only presentation, syncs the updated managed-device flow copy into `translations/en.json` with a regression test that keeps the English translation file aligned to `strings.json`, and routes candidate discovery through a shared ranking helper so Managed Devices surfaces recommend stronger promotion targets before helper-style fallbacks
- **what changed:** repo-side UI cleanup in this run promotes the managed-fleet workspace sensors out of `EntityCategory.DIAGNOSTIC`, the fleet-console button is no longer diagnostic-only, the shipped English translation now includes the managed/unmanaged snapshots and explicit promotion-path wording added to the source strings, `tests/test_translation_sync.py` now fails if `translations/en.json` drifts from `strings.json`, and the Configure Managed Devices screen, fleet console, and managed-device workspace sensors now share one ranked unmanaged-candidate order so the top candidate and shortlist no longer default to alphabetical entity order
- **validation status:** repo-side change verified with `python3 -m unittest tests.test_candidate_utils tests.test_sensor_entity_categories tests.test_button_entity_categories tests.test_translation_sync` plus `python3 -m py_compile custom_components/zero_net_export/candidate_utils.py custom_components/zero_net_export/sensor.py custom_components/zero_net_export/button.py custom_components/zero_net_export/config_flow.py`; live Home Assistant validation is still required before closure, and an SSH re-check on 2026-04-15 still showed the live HA install manifest at `0.1.81`
- **next action:** deploy this candidate to Home Assistant and verify the Managed Devices workspace now surfaces as a first-class native fleet review path with updated managed/unmanaged copy and ranked promotion candidates visible in the live Configure flow instead of stale text or alphabetical pick order
