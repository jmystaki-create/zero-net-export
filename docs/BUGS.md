...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate in this run. The live Home Assistant component payload is still a mixed `0.1.86` install, but this run removed the stale repo-side `0.1.85` manifest/changelog/test drift so the remaining mismatch set is now narrowed to the actual undeployed file difference: live `sensor.py` is still behind repo HEAD.
- **expected behavior:** before trusting further live validation, the Home Assistant install should match one exact current repo candidate across the full shipped component surface, including `manifest.json`, `sensor.py`, and the active `0.1.86` release line.
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo-side metadata correction, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `e9c700c` reported `overall_match=false` with live `manifest.json` `sha256_12=62bcb7ffe76b` (`version: 0.1.86`) versus repo `sha256_12=14a428ae3934` (`version: 0.1.85`), plus live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sha256_12=5c485eb35c56` (40202 bytes). This same run also re-confirmed that the live install itself is already on `0.1.86`, so the repo candidate needed to be brought back onto that same release line instead of continuing to target `0.1.85`.
- **suspected cause:** the project steering and live install have already moved to the `0.1.86` line, but repo metadata and helper expectations had drifted back to `0.1.85`, which made the exact-build candidate itself disagree with the documented live release boundary before deploy.
...
- **validation status:** repo-side fingerprint coverage remains implemented and verified, and this run re-confirmed the live mismatch via the documented HA SSH path before correcting the repo-side version drift. After the metadata fix, the real release blocker is now the undeployed `sensor.py` delta rather than a stale repo version line. Full live closure still requires redeploying one exact `0.1.86` repo build and rerunning fingerprint validation until it reports `overall_match=true`.
