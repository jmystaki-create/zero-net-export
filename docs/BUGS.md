...
- **repo fix:** this run updates `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py` to derive tracked files from the actual shipped component tree, skipping only `__pycache__`; tests now assert coverage for newer shipped files like `binary_sensor.py`, `repairs.py`, `select.py`, and `switch.py` so future UI/runtime files cannot silently fall outside exact-build validation.
- **validation status:** repo-side fix implemented and verified, and live drift is now confirmed against the widened shipped-file fingerprint. This bug can only close after the exact current repo candidate is redeployed and the widened validator returns `overall_match=true`.
