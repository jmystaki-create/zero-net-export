...
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The live system remains materially closer to the repo candidate than before, but it is still not in a trustworthy release state because the exact-build fingerprint still has one undeployed file: live Home Assistant is on `0.1.86`, yet `sensor.py` is still behind repo HEAD.
- **expected behavior:** after install, Zero Net Export should load cleanly, allow Add Integration, avoid integration-specific live errors in Home Assistant logs, and match the exact repo candidate under review, including the current `sensor.py` fleet-workspace fixes.
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against current repo commit `753111b` (expected shipped component commit `c94c19b`) still reports `overall_match=false` with a single remaining shipped-file mismatch, and the mismatch is still only `sensor.py`: live `sensor.py` (`sha256_12=6c87c053febb`, 37812 bytes) versus repo `sensor.py` (`sha256_12=1955e1319cee`, 41548 bytes). The same run still shows `manifest.json` matching live at `0.1.86`, and a quick `ha core logs -n 80 | grep -i zero_net_export` recheck only returned the standard custom-integration warning rather than a new Zero Net Export traceback.
- **suspected cause:** the original install-health failure overlapped ZNE-002 and ZNE-011; the remaining live-health concern now overlaps ZNE-022 because Home Assistant is still running an older `sensor.py` than the current repo candidate even after the repo release metadata was realigned to the live `0.1.86` line.
- **validation status:** materially improved versus the earlier broken install, and the previous manifest/version mismatch is still cleared in this run. The latest HA log spot-check did not surface a new Zero Net Export traceback, but this umbrella bug should stay open until the remaining `sensor.py` drift is reconciled under the formal release flow and a post-restart log review confirms the live install is healthy and fingerprint-aligned.
- **next action:** treat the remaining undeployed `sensor.py` drift under ZNE-022 as the real release blocker, then close this umbrella bug once the live install matches the intended repo candidate end to end and post-restart health stays clean
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. A fresh `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against current repo commit `753111b` still reports `overall_match=false` with `manifest_version_matches=true`, `expected_component_commit=c94c19b`, and the same single remaining mismatch: live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sensor.py` `sha256_12=1955e1319cee` (41548 bytes). This run's documented live API recheck still lines up with that exact drift: `sensor.zero_net_export_managed_fleet_overview` remains `0 managed | 163 unmanaged | top AC Outlet 2`, `sensor.zero_net_export_command_center_status` still reports missing required source roles, and `sensor.zero_net_export_fleet_console_next_step` still incorrectly sends the operator to `Configure -> Managed Devices`, so the new local `repair sources first` fleet-overview wording and source-gated fleet next step remain repo-only until one exact `0.1.86` repo build is redeployed.
...
- **validation status:** repo-side fingerprint coverage remains implemented and verified, and this run re-confirmed through both the documented HA SSH fingerprint path and the live Home Assistant API that the remaining blocker is still one undeployed `sensor.py`. The manifest/version drift stays resolved, but full live closure still requires redeploying one exact `0.1.86` repo build and rerunning fingerprint validation until it reports `overall_match=true`.
