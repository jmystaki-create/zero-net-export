...
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again, and `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `67ba466` reports `overall_match=false` with a single remaining shipped-file mismatch: live `sensor.py` (`sha256_12=6c87c053febb`, 37812 bytes) versus repo `sensor.py` (`sha256_12=5c485eb35c56`, 40202 bytes). The same run shows `manifest.json` now matches live at `0.1.86`.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. A fresh `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against current repo commit `c94c19b` still reports `overall_match=false` with `manifest_version_matches=true` and the same single remaining mismatch: live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sensor.py` `sha256_12=1955e1319cee` (41548 bytes). This run's live API/fingerprint evidence still lines up with that exact drift: the Home Assistant install remains on the older fleet-surface `sensor.py`, so the new local `repair sources first` fleet-overview wording and attributes are still repo-only until one exact `0.1.86` repo build is redeployed.
