...
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded, `ha core logs -n 220 | grep -i zero_net_export` returned the known `entity.py` blocking `read_text` / `open` warnings at `2026-04-16 10:03`, and `scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` with the mismatch narrowed to `entity.py`, so the install-health blocker is now primarily exact-build drift rather than a new runtime traceback.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The current exact-build mismatch set is now the still-older live `entity.py` plus this run's newly updated `sensor.py`, and the matching HA log warning on the older live `entity.py` confirms the repo-side release blocker is deployment rather than another hidden repo regression. The real boundary is now explicit release approval to redeploy the shipped `0.1.85` candidate end to end and rerun fingerprint validation until it returns `overall_match=true`.
