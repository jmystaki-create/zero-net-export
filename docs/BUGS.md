...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate from this run. The live Home Assistant install is much closer to repo HEAD than before, but it is still not the exact code under review: the live install is on manifest `0.1.85`, while the repo candidate is back on `0.1.83`, and `entity.py` is also still mismatched.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reports live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and only two current tracked-file mismatches: `entity.py` and `manifest.json`. The same run reports both `expected_commit` and `expected_component_commit` as `c8cd692`, matching the current shipped-component repo candidate. This materially narrows the drift from the larger earlier mismatch set, but exact-build trust is still not back.
- **suspected cause:** repo HEAD and the live Home Assistant component tree have partly converged again, but the installed package still carries the older `0.1.85` manifest plus the older `entity.py` implementation that performs the blocking manifest read.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The bug state is now refreshed to the current narrower mismatch set (`entity.py`, `manifest.json`) and current shipped-component target commit (`c8cd692`). The real boundary is still explicit release approval to redeploy the shipped candidate and rerun fingerprint validation until it returns `overall_match=true`.
