...
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The live system still appears materially healthier than before, but this run's `ha core logs -n 160 | grep -i zero_net_export` pass still shows the known blocking-I/O warnings from the older live `entity.py`, and fingerprint validation now confirms the installed component is still behind repo HEAD in `candidate_utils.py`, `entity.py`, and `sensor.py`, so install health is not fully back until the exact repo candidate is redeployed.
...
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded, `ha core logs -n 160 | grep -i zero_net_export` returned the known `entity.py` blocking `read_text` / `open` warnings at `2026-04-16 10:03`, and `scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` with shipped-file mismatches in `candidate_utils.py`, `entity.py`, and `sensor.py`, so the install-health blocker is now primarily exact-build drift rather than a new runtime traceback.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo metadata fix, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reported live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and mismatches in `entity.py` plus `manifest.json`. A later recheck narrowed the live install drift to `entity.py` only (`/homeassistant/custom_components/zero_net_export/entity.py` `sha256_12=2dffb9a4ef0c`, repo `sha256_12=9b087d27adff`) while HA logs still showed the known blocking-I/O warnings from that older file. In this run, a fresh fingerprint rerun against repo commit `dae575a` reports live manifest `0.1.85`, `overall_match=false`, and shipped-file mismatches in `candidate_utils.py` (`755215fedf19` live vs `bbffc1f2d809` repo), `entity.py` (`2dffb9a4ef0c` live vs `9b087d27adff` repo), and `sensor.py` (`c7218b24e49b` live vs `10c72541f230` repo), which is expected until the current repo candidate is redeployed end to end.
- **suspected cause:** repo HEAD and the live Home Assistant component tree have mostly converged again, but the installed package still carries older `candidate_utils.py`, `entity.py`, and `sensor.py` implementations than repo HEAD.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The current exact-build mismatch set is now the still-older live `candidate_utils.py`, `entity.py`, and `sensor.py`, and the matching HA log warning on the older live `entity.py` confirms the repo-side release blocker is deployment rather than another hidden repo regression. The real boundary is now explicit release approval to redeploy the shipped `0.1.85` candidate end to end and rerun fingerprint validation until it returns `overall_match=true`.
