...
- **repo fix:** earlier work updated `custom_components/zero_net_export/sensor.py` so every configured load now exposes a primary `managed summary` sensor on the Zero Net Export device page, while deeper per-device runtime history sensors such as last-requested power, last-applied power, durations, timestamps, and last-action detail move into diagnostic categorization. This same bug fix path also renamed the per-device review buttons in `custom_components/zero_net_export/button.py` to explicit `{device} managed review` labels so the device-page managed-device actions group more clearly. A later pass tightened the device-page surface again by moving per-device status, guard status, planned action, and planned power delta sensors into `EntityCategory.DIAGNOSTIC`, leaving the primary device page focused on each load's managed summary plus live power/target entities. `56a1278` now advances that cleanup again by moving redundant runtime-health, source-stale, and per-device usable binary sensors into diagnostic categorization in `custom_components/zero_net_export/binary_sensor.py`, so the primary device page keeps the stronger managed-device summary/action rows while the boolean troubleshooting detail stays secondary. This run further strengthens that primary row in `custom_components/zero_net_export/sensor.py` so the managed summary itself now carries device kind, current target power for variable loads, and non-nominal guard/last-action signals instead of hiding those cues behind secondary diagnostic entities.
- **validation status:** repo-side fix advanced again in this run and is now covered by `python3 -m unittest tests.test_sensor_entity_categories tests.test_button_entity_categories tests.test_binary_sensor_entity_categories -q` plus `python3 -m unittest discover -s tests -q`. This run adds explicit summary-row regression coverage in `tests/test_sensor_entity_categories.py` for both fixed and variable managed devices, including variable target power and blocked-guard visibility in the primary managed summary row. Live Home Assistant validation is still pending on the next exact-build redeploy.
...
- **evidence:** documented Home Assistant API inspection originally returned `sensor.zero_net_export_command_center_status = Missing required source roles: Solar power, Solar energy, Grid import power, Grid export power, Grid import energy, Grid export energy` while `sensor.zero_net_export_fleet_console_next_step = Open Settings -> Devices & Services -> Integrations -> Zero Net Export -> Configure -> Managed Devices and tag the first candidate into the fleet`. This run rechecked the same live API path and the mismatch is still present on the current installed build: `sensor.zero_net_export_managed_fleet_overview = 0 managed | 163 unmanaged | top AC Outlet 2`, `sensor.zero_net_export_command_center_status` still reports the same required source-role blockers, and `sensor.zero_net_export_fleet_console_next_step` still incorrectly sends the operator to `Configure -> Managed Devices` first.
...
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The live system remains materially closer to the repo candidate than before, but it is still not in a trustworthy release state because the exact-build fingerprint still has two undeployed files: live Home Assistant is on `0.1.86`, yet `native_support.py` and `sensor.py` are still behind repo HEAD.
- **expected behavior:** after install, Zero Net Export should load cleanly, allow Add Integration, avoid integration-specific live errors in Home Assistant logs, and match the exact repo candidate under review, including the current `native_support.py` command-center gating fix and the `sensor.py` fleet-workspace fixes.
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against current repo commit `f11ac60` now reports `overall_match=false` with two remaining shipped-file mismatches: live `native_support.py` (`sha256_12=872e48901843`, 69325 bytes) versus repo `native_support.py` (`sha256_12=ed7dec7d65e0`, 69543 bytes), and live `sensor.py` (`sha256_12=6c87c053febb`, 37812 bytes) versus repo `sensor.py` (`sha256_12=1955e1319cee`, 41548 bytes). The same run still shows `manifest.json` matching live at `0.1.86`, a quick `ha core logs -n 80 | grep -i zero_net_export` recheck only returned the standard custom-integration warning rather than a new Zero Net Export traceback, and the documented live Home Assistant API still shows `sensor.zero_net_export_command_center_status` recommending `Managed Devices` first even while required source roles are missing.
- **suspected cause:** the original install-health failure overlapped ZNE-002 and ZNE-011; the remaining live-health concern now overlaps ZNE-022 because Home Assistant is still running older `native_support.py` and `sensor.py` files than the current repo candidate even after the repo release metadata was realigned to the live `0.1.86` line.
- **validation status:** materially improved versus the earlier broken install, and the previous manifest/version mismatch is still cleared in this run. The latest HA log spot-check did not surface a new Zero Net Export traceback, but this umbrella bug should stay open until the remaining `native_support.py` and `sensor.py` drift is reconciled under the formal release flow and a post-restart log review confirms the live install is healthy and fingerprint-aligned.
- **next action:** treat the remaining undeployed `native_support.py` and `sensor.py` drift under ZNE-022 as the real release blocker, then close this umbrella bug once the live install matches the intended repo candidate end to end and post-restart health stays clean
...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate in this run. The live Home Assistant component payload is now aligned on the `0.1.86` release line, but the remaining mismatch set is no longer just one file: live `native_support.py` and `sensor.py` are both still behind repo HEAD.
- **expected behavior:** before trusting further live validation, the Home Assistant install should match one exact current repo candidate across the full shipped component surface, including the current `native_support.py`, `sensor.py`, and the active `0.1.86` release line.
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. A fresh `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against current repo commit `f11ac60` still reports `overall_match=false` with `manifest_version_matches=true`, `expected_component_commit=f11ac60`, and two remaining mismatches: live `native_support.py` `sha256_12=872e48901843` (69325 bytes) versus repo `native_support.py` `sha256_12=ed7dec7d65e0` (69543 bytes), and live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sensor.py` `sha256_12=1955e1319cee` (41548 bytes). This run's documented live API recheck still lines up with that exact drift: `sensor.zero_net_export_command_center_status` still reports missing required source roles while its `device_next_step` incorrectly sends the operator to `Configure -> Managed Devices`, `sensor.zero_net_export_managed_fleet_overview` still reads `0 managed | 163 unmanaged | top AC Outlet 2`, and `sensor.zero_net_export_fleet_console_next_step` still incorrectly sends the operator to `Configure -> Managed Devices`, so the new command-center blocker gating plus the `repair sources first` fleet-overview wording remain repo-only until one exact `0.1.86` repo build is redeployed.
- **suspected cause:** the project steering and live install are now aligned on the `0.1.86` line, but Home Assistant is still running older deployed `native_support.py` and `sensor.py` files, so the exact-build candidate and live install still disagree even after the repo metadata correction landed.
- **repo fix:** `afbbd71`, `cf46eac`, `b9766aa`, `f45aef9`, `c94c19b`, `f11ac60` — widen exact-build fingerprint coverage beyond the old hand-maintained subset, then derive tracked files from the actual shipped component tree in `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py`, skipping only `__pycache__`; tests now assert coverage for active shipped files like `button.py`, `candidate_utils.py`, `binary_sensor.py`, `repairs.py`, and `select.py` so future UI/runtime files cannot silently fall outside exact-build validation. This run also realigns `custom_components/zero_net_export/manifest.json`, `CHANGELOG.md`, `README.md`, and `tests/test_install_helper_scripts.py` to `0.1.86`, adds the fleet-overview source-blocker visibility tracked under `c94c19b`, and gates command-center `device_next_step` on install-provenance or source blockers in `f11ac60` so the repo candidate again matches the intended current live correction line.
- **validation status:** repo-side fingerprint coverage remains implemented and verified, and this run re-confirmed through both the documented HA SSH fingerprint path and the live Home Assistant API that the remaining blockers are still undeployed `native_support.py` and `sensor.py`. The manifest/version drift stays resolved, but full live closure still requires redeploying one exact `0.1.86` repo build and rerunning fingerprint validation until it reports `overall_match=true`.
- **next action:** ask James directly to approve the formal `0.1.86` release-reconciliation flow now, then deploy the exact current repo candidate so the remaining `native_support.py` and `sensor.py` drift is cleared, rerun fingerprint validation until it reports `overall_match=true`, restart Home Assistant, and continue Configure/native-UI validation on the exact installed candidate
