...
- **current observed behavior:** the main drift confirmed in this watchdog run was bug-tracker drift, not a fresh repo-side UI regression. Current `HEAD` already ships separate `Top alerts` and `Recommended next step` lines in `custom_components/zero_net_export/native_support.py`, and `custom_components/zero_net_export/button.py` already splits `Review managed devices workspace` into `Managed devices needing attention first` and `Other managed devices`. `docs/BUGS.md` was still describing those older gaps as active, which understated current repo progress and risked sending watchdog runs back into stale bookkeeping instead of the next unfinished `0.1.87` workstream step.
...
- **evidence:** repo/design audit against `docs/UI_DESIGN.md` and `docs/UI_IMPLEMENTATION_MAP.md`, plus current repo inspection on `181ba3f`. `rg -n "Top alerts|Recommended next step|Fleet activity" custom_components/zero_net_export/native_support.py tests` shows the split alert/next-step wording is already shipped and covered by `tests/test_command_center_setup_focus.py` and `tests/test_command_center_modal_copy.py`. `custom_components/zero_net_export/button.py` already builds `Managed devices needing attention first:` and `Other managed devices:` for `Review managed devices workspace`, and `tests/test_button_entity_categories.py` still expects the current device-page Managed Devices wording. This run therefore updates the bug entry to stop treating those resolved repo-side gaps as still-open evidence.
...
- **repo fix:** `9c988a8`, `2603de9`, `f804286`, `827530c`, `5c4ff5c`, `0acb62e`, `96fe3d2`, `28c1834`, `45c61ec`, `55c68bd`, `e9e0189`, `a0046ee`, `7cd6c35`, `b6ddcd2`, `c41929c`, `23f2e9c`, `8ad2e88`, `9e25578`, `469825e`, `f45618c`, `a67acdc`, `261e7b7`, `dae575a`, `c94c19b`, `cc24e33`, `fcf20e6`, `1477b80`, `5eaa529`, `6013f0a`, `50dc07c`, `e89324f`, `fca6632`, `245eb09`, `7d92a22`, `69134a7`, `5d49a2c`, `5925f6f`, `2514ef7`, `02a3e8d`, `08b7994`, `b812e36`, `f4b6f09`, `5186f80`, `f0ef24e`, `8e3759a`, `72a487f`, `4c52c11`, `0fc3444`, `1aceae9`, `71b5f9c`, `fc5edaa`, `26aca12`, `f1b8745`, `d50a4b6`, `0a5ca32`, `c3fdcd2`, `c616150`, `1f64ae6`, `436104b`, `4abbbe5`, `8a62c98`, `d543833`, `202844d`, `859af18`, `9fb7f63`, `53fa040`, `d888c4c`, `4f113ac`, `85e6ef9`, `3f91c5d`, `e7fdc0f`, `d6caab4`, `fe5b0df`, `6e273f3`, `44a5bc3`, `02a7ed7`, `58e4662`, `ba7a625`, `26b68a2`, `3291ed9`, `49af82f`, `078a04a`, `5f0aa87`, and `181ba3f`. This run adds a source-of-truth follow-up that refreshes `docs/BUGS.md` itself so resolved repo-side gaps are no longer tracked as still-open evidence.
- **validation status:** repo-side UI changes remain covered by `python3 -m unittest -q tests.test_command_center_setup_focus tests.test_command_center_modal_copy tests.test_button_entity_categories`, and `python3 -m unittest discover -q` still passes on the current repo candidate. The documented HA SSH fingerprint path was already rechecked against the current repo candidate before this suffix and still reports the same known exact-build mismatch under ZNE-022, so unchanged fingerprint refresh is not the right default next step for this bug.
- **next action:** keep this bug in `fixed_pending_validation`, but do not spend another watchdog run re-reporting these resolved repo-side gaps. The next most important gap is still the next unfinished repo-side `0.1.87` workstream step in `docs/UI_IMPLEMENTATION_MAP.md`, not another unchanged fingerprint refresh.
...
- **current observed behavior:** the earlier broken-config-flow and missing-entity install state is no longer the current live symptom. The documented HA SSH path still works, but the live system remains materially behind the repo candidate and is not in a trustworthy release state: the latest exact-build fingerprint now shows a smaller but still release-blocking **nine-file** drift set against validation anchor `b76776b`. Live Home Assistant is back on `0.1.87`, while `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` are all behind the current repo state.
- **expected behavior:** after install, Zero Net Export should load cleanly, allow Add Integration, avoid integration-specific live errors in Home Assistant logs, and match the exact repo candidate under review, including the current managed-device workspace/review grouping fixes plus the latest `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, and translation changes.
- **evidence:** live HA review on 2026-04-15 originally confirmed remote manifest `0.1.83` plus `homeassistant.config_entries` errors and the missing-config-entry/orphaned-entity condition tracked elsewhere in this file. In the last live validation run, documented HA SSH access on `root@192.168.86.200:2222` succeeded and `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `f5f1715` reported `overall_match=false` with `config_flow.py`, `native_support.py`, and `sensor.py` still behind. In this watchdog run, the same documented HA SSH path still succeeds and the fingerprint result is still not aligned with the current repo candidate: `overall_match=false`, `manifest_version_matches=false`, against validation anchor `b76776b`, with live `button.py` (`sha256_12=9d4b517e6deb`, 52690 bytes) versus repo `button.py` (`sha256_12=38ec0090710c`, 55515 bytes), live `candidate_utils.py` (`sha256_12=2ab5d6d837fe`, 32137 bytes) versus repo `candidate_utils.py` (`sha256_12=a71fd71f6dda`, 33037 bytes), live `config_flow.py` (`sha256_12=9c60dc18f33b`, 153684 bytes) versus repo `config_flow.py` (`sha256_12=0fe21c693040`, 156832 bytes), live `diagnostics.py` (`sha256_12=f16c7a91922a`, 10313 bytes) versus repo `diagnostics.py` (`sha256_12=207e795b90c2`, 10300 bytes), live `manifest.json` (`sha256_12=e6af6a9ba935`, 461 bytes) versus repo `manifest.json` (`sha256_12=62bcb7ffe76b`, 461 bytes), live `native_support.py` (`sha256_12=64eea967d4bb`, 84282 bytes) versus repo `native_support.py` (`sha256_12=b3d67dfc4ec5`, 88767 bytes), live `sensor.py` (`sha256_12=2d55cd2d424d`, 46827 bytes) versus repo `sensor.py` (`sha256_12=13505dcedc6d`, 48453 bytes), live `strings.json` (`sha256_12=7485dab01108`, 29339 bytes) versus repo `strings.json` (`sha256_12=01264b2449d9`, 29424 bytes), and live `translations/en.json` (`sha256_12=7485dab01108`, 29339 bytes) versus repo `translations/en.json` (`sha256_12=01264b2449d9`, 29424 bytes). The same comparison shows other tracked files still aligned, so this is no longer the older ten-file drift set. The live manifest is also back ahead of repo at `0.1.87`, so the release line is misaligned again rather than merely carrying undeployed component files.
- **suspected cause:** the original install-health failure overlapped ZNE-002 and ZNE-011; the remaining live-health concern now overlaps ZNE-022 because Home Assistant is still running an older mixed candidate with newer release metadata, leaving `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` plus `manifest.json` out of sync with the current repo candidate.
- **validation status:** materially improved versus the earlier broken install, but the previous manifest/version-mismatch clearance is no longer true in this run. The latest HA SSH fingerprint recheck shows a smaller nine-file mixed-build drift set plus `manifest_version_matches=false` again, so this umbrella bug should stay open until the remaining `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` drift is reconciled under the formal release flow and a post-restart log review confirms the live install is healthy and fingerprint-aligned. Repeating the same fingerprint check again without a new deploy/restart or explicit release authorization would now be loop churn, not new validation.
- **next action:** treat the remaining `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` drift under ZNE-022 as the real release blocker, and ask James directly for explicit deploy/restart approval before another formal release-validation pass. If release authorization is still not available, keep `0.1.87` repo-side UI work moving instead of re-running unchanged fingerprint bookkeeping.
...
- **current observed behavior:** this watchdog run rechecked the documented HA SSH path with `python3 /root/.openclaw/workspace/projects/zero-net-export/scripts/validate_install_fingerprint.py --ssh-host root@192.168.86.200 --ssh-port 2222 /config` and the live install is still drifted, not aligned. The comparison now reports `overall_match=false`, `manifest_version_matches=false`, live `manifest_version=0.1.87` versus repo `0.1.86`, with validation anchor `b76776b`, plus tracked-file mismatches in `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`.
...
- **evidence:** documented HA SSH access succeeded in this run, so the blocker is not access. The fresh comparison payload at `/root/.openclaw/workspace/projects/zero-net-export/tmp/install-fingerprint-compare.json` now shows the reopened drift directly, including the live `0.1.87` manifest and the nine tracked-file mismatches above against validation anchor `b76776b`. Other tracked files are aligned again, so the active blocker has narrowed even though exact-build validation is still red. That still leaves screenshot-grade live validation behind the real release boundary: James must explicitly approve deploy/restart of one exact current repo build before live UX validation can become the main next step again.
- **next action:** ask James directly for explicit deploy/restart approval, then redeploy one exact repo build and rerun fingerprint validation before treating live UX validation as the main next step again. If approval is still not available, keep shipping the next mapped `0.1.87` repo-side fix instead of rechecking the same unchanged drift.
