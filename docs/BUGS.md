...
- **current observed behavior:** earlier live inspection showed no active `zero_net_export` config entry and no controller device/entities after restart. In this run that specific condition is no longer present: the live install now has an active `zero_net_export` config entry again and Zero Net Export device-registry records are present, but follow-on Configure and sensor-state bugs remain.
...
- **evidence:** earlier live SSH/API inspection showed `0` `zero_net_export` config entries, `0` Zero Net Export devices, and only `update.zero_net_export_update` visible. In this run, documented HA SSH access on `root@192.168.86.200:2222` succeeded again and direct inspection of `/config/.storage/core.config_entries` now shows one `zero_net_export` entry (`01KP8MW539MQ724BBFZX2EF6S2`, title `Zero Net Export`, created `2026-04-15T13:21:46.601855+00:00`). Remote inspection of `/config/.storage/core.device_registry` now also finds Zero Net Export records again.
...
- **validation status:** improved materially but not yet closed. The exact missing-config-entry condition is no longer reproducible live, but restart persistence still needs re-validation before this bug can be closed confidently.
- **next action:** keep this bug at `fixed_pending_validation`, then re-test restart/reload persistence after the current Configure and sensor-state regressions are fixed so the entry/device/entity set is proven stable across another live restart
...
- **current observed behavior:** startup logs had warned that Zero Net Export performed blocking `read_bytes` and `open` calls inside the event loop while building install-provenance metadata from `manifest.json`
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded in the earlier validation run, and post-restart `ha core logs -n 120` showed `Detected blocking call to read_bytes ... by custom integration 'zero_net_export' at custom_components/zero_net_export/release_info.py, line 62: sha256_12 = hashlib.sha256(path.read_bytes()).hexdigest()[:12]` plus the corresponding blocking `open` warning from the same code path. In this run, the updated `release_info.py` was copied to `/config/custom_components/zero_net_export`, the remote `sha256sum` for that file matched the local repo build exactly, Home Assistant core was restarted again at `00:30`, and a fresh `ha core logs -n 220` review after startup no longer showed any new `Detected blocking call` or `release_info.py` warnings after the `00:31` boot sequence.
- **suspected cause:** `build_install_provenance()` ultimately called `_cached_install_provenance()`, which still fell back to synchronous manifest/hash reads from `release_info.py` when the earlier warmup path had not produced a stable shared snapshot for later sync consumers on the event loop
...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate from this run. The live Home Assistant install is behind the current repo candidate across multiple shipped UI files, so the exact installed package is still not the exact code under review.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded in this run. The widened validator now fingerprints all shipped `.py` and `.json` component files instead of a hand-maintained subset, and `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reported live manifest `0.1.83` with `overall_match=false`, including mismatches at `button.py`, `candidate_utils.py`, `config_flow.py`, `release_info.py`, `strings.json`, and `translations/en.json`. Focused regression coverage passed in `python3 -m unittest tests.test_install_helper_scripts tests.test_release_info_install_guidance -q`, and the full repo suite also passed in `python3 -m unittest discover -s tests -q` after the fingerprint helpers and release-info provenance snapshot were updated to enumerate the shipped file set dynamically.
- **suspected cause:** repo HEAD advanced after the earlier live deploy/restart cycle, and the older fingerprint subset could miss drift in newer shipped files unless every tracked file list stayed manually synchronized.
- **repo fix:** `afbbd71`, `cf46eac`, `b9766aa` — widen exact-build fingerprint coverage beyond the old hand-maintained subset, then derive tracked files from the actual shipped component tree in `scripts/print_expected_install_fingerprint.py`, `scripts/compare_install_fingerprint.py`, and `custom_components/zero_net_export/release_info.py`, skipping only `__pycache__`; tests now assert coverage for active shipped files like `button.py`, `candidate_utils.py`, `binary_sensor.py`, `repairs.py`, `select.py`, and `switch.py` so future UI/runtime files cannot silently fall outside exact-build validation.
- **validation status:** repo-side fix implemented and verified, and live drift is now confirmed against the widened shipped-file fingerprint. Repo state itself is no longer the blocker in this run: `git rev-list --left-right --count @{upstream}...HEAD` returned `0 0`, so the remaining boundary is explicit release approval to redeploy the exact current candidate and rerun fingerprint validation until it returns `overall_match=true`.
- **next action:** ask James directly for release approval to redeploy the exact current repo build from this run, rerun fingerprint validation until it reports `overall_match=true`, then restart Home Assistant and continue Configure/native-UI validation on the exact installed candidate
