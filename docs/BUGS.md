...
- **validation status:** repo-side fix implemented in this run and verified with `python3 -m unittest tests.test_device_surface_version -q` plus `python3 -m unittest discover -s tests -q`. Live Home Assistant validation is still pending on the next exact-build redeploy because the current installed fingerprint remains behind repo HEAD.
...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate from this run. After refreshing the repo working version back to the source-of-truth `0.1.85` release line, the live Home Assistant install is now down to one exact-build mismatch: `entity.py` is still older than repo HEAD.
...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo metadata fix, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reported live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and mismatches in `entity.py` plus `manifest.json`. This run corrected the repo manifest/changelog/test expectations back to `0.1.85`, so the exact-build drift has narrowed to the still-older live `entity.py`; fingerprint trust is still not back until that remaining file mismatch is redeployed and rechecked.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. This run also corrected the repo-side version drift back to `0.1.85`, so the remaining exact-build mismatch set is now just the still-older live `entity.py`. The real boundary is still explicit release approval to redeploy the shipped candidate and rerun fingerprint validation until it returns `overall_match=true`.
- **next action:** ask James directly for release approval to redeploy the exact current shipped-component `0.1.85` candidate, rerun fingerprint validation until it reports `overall_match=true`, then restart Home Assistant and continue Configure/native-UI validation on the exact installed candidate
