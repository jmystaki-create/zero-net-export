...
- **current observed behavior:** `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports `overall_match=false` against the current repo candidate in this run. The live Home Assistant component payload is still a mixed `0.1.86` install: `manifest.json` remains at `0.1.86`, but `sensor.py` is still behind repo HEAD, so release state and live UI claims are still not trustworthy.
- **expected behavior:** before trusting further live validation, the Home Assistant install should match one exact current repo candidate across the full shipped component surface, including `manifest.json`, `sensor.py`, and the active release line.
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` against repo commit `bfaadff` reported `overall_match=false` with live `manifest.json` `sha256_12=62bcb7ffe76b` (`version: 0.1.86`) versus repo `sha256_12=14a428ae3934` (`version: 0.1.85`), plus live `sensor.py` `sha256_12=6c87c053febb` (37812 bytes) versus repo `sha256_12=5c485eb35c56` (40202 bytes).
- **suspected cause:** the project steering and live install have already moved to the `0.1.86` line, but repo metadata and bug-tracker release guidance were still partially steering toward a `0.1.85` reconciliation path. That leaves the system in a mixed release state where the installed version and the current repo candidate no longer describe the same release boundary.
