...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. Before the repo metadata fix, `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` reported live manifest `0.1.85`, repo manifest `0.1.83`, `overall_match=false`, and mismatches in `entity.py` plus `manifest.json`. This run rechecked the live install after the repo metadata correction, and fingerprint output now shows live manifest `0.1.85` with the mismatch narrowed to `entity.py` only (`/homeassistant/custom_components/zero_net_export/entity.py` `sha256_12=2dffb9a4ef0c`, repo `sha256_12=9b087d27adff`). The same run's HA logs still show the known blocking-I/O warnings from that older live `entity.py`, so fingerprint trust is still not back until that remaining file mismatch is redeployed and rechecked.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. The remaining exact-build mismatch set is now just the still-older live `entity.py`, and the matching HA log warning confirms the repo-side release blocker is deployment rather than another hidden repo regression. The real boundary is now explicit release approval to redeploy the shipped `0.1.85` candidate end to end and rerun fingerprint validation until it returns `overall_match=true`.
