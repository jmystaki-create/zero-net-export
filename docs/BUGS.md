...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports live manifest `0.1.83` with `overall_match=false`, and the mismatches remain in shipped files including `button.py`, `candidate_utils.py`, `config_flow.py`, `native_support.py`, `release_info.py`, `strings.json`, and `translations/en.json`. Repo inspection in this run also confirmed the bug tracker had drifted again on repo-state detail: `git rev-parse --short HEAD` is now `cf9b246`, while `git log --oneline -- custom_components/zero_net_export | head -n 1` still identifies `6f9cff2` as the latest shipped-component commit. Since only `docs/BUGS.md` and `project_status.md` changed after `6f9cff2`, the redeploy target is still `6f9cff2`, but the bug entry needed its repo-HEAD evidence refreshed so release guidance stays accurate.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. Repo state itself is not the blocker: `git status --short` is clean, repo `HEAD` is now `cf9b246`, and the latest shipped-component candidate remains `6f9cff2`; this run refreshes the stale repo-state evidence so the approval boundary still names the actual redeploy target instead of implying repo HEAD itself should be deployed. The real boundary is still explicit release approval to redeploy the shipped candidate and rerun fingerprint validation until it returns `overall_match=true`.
