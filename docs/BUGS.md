...
- **evidence:** documented HA SSH access on `root@192.168.86.200:2222` succeeded again in this run. `python3 scripts/validate_install_fingerprint.py /config/custom_components --ssh-host root@192.168.86.200 --ssh-port 2222` still reports live manifest `0.1.83` with `overall_match=false`, but the exact mismatch set has drifted again and the bug tracker needed to be refreshed: the current live mismatches are `button.py`, `config_flow.py`, `coordinator.py`, `entity.py`, `native_support.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`, while earlier mismatch items such as `candidate_utils.py`, `diagnostics.py`, and `release_info.py` now match the repo candidate again. The same fingerprint run now emits both `expected_commit` and `expected_component_commit` as `07bfd5a`, matching current shipped-component HEAD (`ui: tighten managed device page signal`). The release target has therefore moved forward again with the shipped component tree, not just docs-only repo HEAD.
...
- **validation status:** repo-side fix remains implemented and verified, and live drift is re-confirmed in this run against the widened shipped-file fingerprint. This run's concrete progress is bug-tracker-state repair: the tracked mismatch list and expected shipped-component commit were stale relative to the current fingerprint output, and they now explicitly match the current `07bfd5a` repo candidate. The real boundary is still explicit release approval to redeploy the shipped candidate and rerun fingerprint validation until it returns `overall_match=true`.
