...
- stabilize install/runtime behavior in Home Assistant ✅ live HA now reaches a loaded state on the current `0.1.82` repo release line, with native entities present again after the startup crash chain was narrowed and deferred release-metadata reads were fixed on hot paths
