...
2. **Managed-device list must be managed-only**
...
3. **Managed rows need the settings gear in the native right-side action location / visible right-side action location**
...
4. **Unmanaged candidates stay behind workflow/backlog surfaces**
...
5. **Per-service service-card actions for multi-plan operation**
...
6. **Managed-load row three-dot delete action**
...
7. **Managed-load configuration card with editable captured settings**
...
8. **No release-readiness claim without proof**
...
9. **No direct Home Assistant updates outside release management**
