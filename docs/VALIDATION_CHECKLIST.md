...
5. The `v0.1.90` Home Assistant install/reload and fingerprint boundary is now complete: the live API reports `sensor.zero_net_export_installed_version = 0.1.90`, and the documented SSH fingerprint helper reported `overall_match: true` against `d94436a`. The remaining boundary is screenshot-grade device-page evidence capture and the Managed Devices action drill-down check.
...
For `0.1.90`, with the Home Assistant install/update plus restart/reload/fingerprint step now complete, validate in this order:
