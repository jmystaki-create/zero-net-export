...
- [ ] The installed package fingerprint shown in Home Assistant matches the expected component commit (`expected_component_commit` / `preferred_validation_commit`) and tracked-file hashes from `python3 scripts/print_expected_install_fingerprint.py --write-json tmp/expected-install-fingerprint.json`
