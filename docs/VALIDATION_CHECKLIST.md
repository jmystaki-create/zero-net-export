...
5. The repo later froze `4c0d071` / `v0.1.94`, then component code moved again at `0b4f420`, after the earlier `db5c246` / `v0.1.92` and `026f189` / `v0.1.93` freezes while this checklist still belongs to the documented `0.1.91` scope; before any Home Assistant install, restart, fingerprint validation, or screenshot claim, ask James directly whether current `0b4f420` / manifest `0.1.94` replaces the documented `0.1.91` release target or whether release execution returns to the approved `0.1.91` boundary.
