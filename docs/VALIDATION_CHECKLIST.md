...
- If you need to overwrite a mixed manual install from one exact repo build first, run `python3 scripts/deploy_exact_repo_build.py /path/to/home-assistant/config` (or point it at `custom_components` or `custom_components/zero_net_export`). It replaces the destination component directory instead of layering files onto an older copy, keeps a timestamped backup by default, and then runs the same fingerprint validation against the deployed path before restart.
