...
5. The repo later froze `db5c246` / `v0.1.92` while this checklist still belongs to the documented `0.1.91` scope; before any Home Assistant install, restart, fingerprint validation, or screenshot claim, ask James directly whether `v0.1.92` replaces the documented `0.1.91` release target or whether release execution returns to the approved `0.1.91` boundary.
