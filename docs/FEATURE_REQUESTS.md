...
- **validation result:** repo validation passed `node --check`, Python compile for changed backend, focused Managed Devices tests, focused candidate/device tests, full discovery (`637` tests), and `git diff --check`. Release/live proof through GitHub/HACS and Slave browser remains pending.
