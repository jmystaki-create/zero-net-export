...
The repo implementation exists inside the approved scope, but component code changed after the historical `v0.1.91` tag at `7217f3b`, then the repo froze `v0.1.92` at `db5c246`, `v0.1.93` at `026f189`, and later `v0.1.94` at `4c0d071` without the governing source-of-truth docs approving a release-target change. The fingerprint helper now resolves the install candidate to `0b4f420` / manifest `0.1.94` after the post-freeze component guidance fix, while this plan still documents the approved `0.1.91` scope.

Do not deploy, restart, fingerprint-validate, or call successful either the old `7217f3b` tag, the post-tag `c4802a3` component boundary, the unapproved `db5c246` / `v0.1.92` candidate, the unapproved `026f189` / `v0.1.93` candidate, the newer `4c0d071` / `v0.1.94` freeze, or the post-freeze `0b4f420` component boundary until James explicitly decides whether current `0b4f420` / manifest `0.1.94` replaces the documented `0.1.91` release target or the project returns to the approved `0.1.91` boundary. After that release-target decision, James still needs to accept the closest native entity/device-info representation and approve deploy/restart validation for the exact target before any Home Assistant install/restart or success claim.
