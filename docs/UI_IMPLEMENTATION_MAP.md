...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper still resolves the exact current component-changing deploy boundary as `6d993f6`. Later commits `129a5a7` and `b0936f0` are docs-only source-of-truth refreshes, not a newer shipped component candidate.
- The strongest concrete drift in this run is watchdog/supervisor loop behavior: after the source-of-truth docs were already corrected to anchor release discussion on `6d993f6`, the next repo movement was another `docs/BUGS.md` fingerprint-evidence refresh on unchanged live drift. That is bookkeeping churn, not a new release-stage fact or a new reason to elevate Workstream G over unfinished mapped A/B/D/F runway.
