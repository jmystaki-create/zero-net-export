...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `77f43be`. The older `129a5a7`, `b0936f0`, and `516e75f` docs updates were bookkeeping-only, but the later `0a83406`, `a03f80f`, `894d211`, `408d7a1`, `732050f`, and `77f43be` commits are real `0.1.87` UI movement in `config_flow.py`, `sensor.py`, `strings.json`, and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `732050f` is stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup through `77f43be`, but parts of the docs were still framing the exact-build boundary around older anchors. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
