...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `46585f7`, not the earlier `33da862` checkpoint. The live mismatch evidence itself is unchanged, but the repo candidate moved again because `84b7e4d` and then `46585f7` landed as real Workstream A/B cleanup after the older boundary notes.
- The strongest concrete drift in this run is source-of-truth lag again: the implementation map was still steering off `33da862` even after `46585f7` preserved the review-ready fleet story under large backlogs. That is exactly the kind of stale release-boundary bookkeeping churn the supervisor warns against, because it keeps older candidate framing in front of still-moving `0.1.87` repo runway.
...
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `46585f7` build is reviewed in live Home Assistant.
