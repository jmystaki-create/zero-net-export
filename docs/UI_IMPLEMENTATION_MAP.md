...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The source-of-truth release anchor had drifted behind the repo again: `python3 scripts/print_expected_install_fingerprint.py` now reports `expected_commit=390a516`, `expected_component_commit=390a516`, `preferred_validation_commit=390a516`, and `repo_head_commit=390a516`, so any watchdog/supervisor update still talking about `65f9b95` or `2c1f7fd` is stale. The live blocker is still one undeployed component build rather than a new product regression, but the exact build to approve/deploy is now `390a516`.
...
- The next gap is to finish or explicitly retire the highest remaining repo-side UI step in workstream order, then judge exact build `390a516` in live Home Assistant instead of elevating unchanged validation bookkeeping ahead of product work. A mismatch refresh without new deploy/restart evidence, a newly changed exact-build anchor, or a newly retired repo-side UI step is churn rather than progress.
