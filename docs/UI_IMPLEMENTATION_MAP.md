...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `f94cd7f`. The earlier docs-only commits `099640d`, `c05c697`, `aef9a2c`, and `b8259a3` were bookkeeping-only, but the more recent component commits `fa182d4`, `facc3af`, and `f94cd7f` are additional real `0.1.87` UI movement in `sensor.py`, `native_support.py`, `strings.json`, and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `facc3af`, `77f43be`, or `732050f` is stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup through `f94cd7f`, but parts of the docs were still framing the exact-build boundary around older anchors. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
