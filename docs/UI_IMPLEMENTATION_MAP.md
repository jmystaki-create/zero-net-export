...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `6d993f6`, not the older `e02c34d` anchor still implied by some source-of-truth notes. Treat that helper output, not a hard-coded commit hash in these docs, as the release anchor.
- The strongest concrete drift in this run is source-of-truth lag: supervisor-facing docs were still describing repo `HEAD` as doc-only churn past `e02c34d` even though `6d993f6` is a new Workstream A/B code change that keeps live managed activity ahead of the unmanaged backlog in the opening command center. That stale framing would mis-rank the current repo candidate and encourage more release-bookkeeping churn instead of tracking the real latest component-changing build.
