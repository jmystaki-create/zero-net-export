...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `47e157f`, not the earlier `bade75f`, so the source-of-truth docs must stop describing post-`bade75f` repo movement as doc-only churn.
- Repo-side `0.1.87` shaping across Workstreams A-F still reads materially complete enough that default watchdog/supervisor cycles should not keep pretending there is a large untouched repo runway ahead of exact-build deploy/restart approval. But the current release-boundary anchor moved again for a real reason: `222ce35` and `47e157f` are component-changing `config_flow.py` follow-ons after `bade75f`, not docs-only bookkeeping.
- The next real boundary is still conditional: if the current repo candidate is being presented as release-ready, ask James directly for explicit deploy/restart approval for exact build `47e157f`; otherwise keep working the next mapped `0.1.87` UI step. Do not keep spending watchdog/supervisor runs on unchanged fingerprint bookkeeping, approval-target retargeting, or doc-only refreshes that do not change the actual release decision.
