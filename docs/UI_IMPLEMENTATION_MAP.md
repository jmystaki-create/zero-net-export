...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact component-changing deploy boundary at `6bbcc5b`, not the older `316aa5b` churn checkpoint. The intervening component commits `17246ba` and `6bbcc5b` are real Managed Devices follow-on work, so source-of-truth text must stop describing the current state as unchanged docs-only churn.
- The strongest concrete drift in this run was source-of-truth staleness: the repo moved forward with more Workstream B/C follow-on code, but this file was still ranking the older release-anchor churn audit as if it were the latest meaningful change.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, not the default build-stage next step while mapped `0.1.87` runway still remains. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary or operator instruction actually changed.
