...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the same six tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`. Live and repo `manifest_version` both still read `0.1.86`, stale backup artifacts remain absent, and `scripts/print_expected_install_fingerprint.py` now resolves the exact component-changing deploy boundary to `a470bbf`, with repo `HEAD` matching that same component-changing commit.
- The strongest drift in this run is stale release-boundary bookkeeping, not a newly widened live blocker: the source-of-truth notes were still anchored on the older `c410598` plus doc-only-`HEAD` story even though two later component commits moved the real candidate to `a470bbf` while the live HA mismatch remained the same six files.
- Keep the ranking lesson intact: live exact-build mismatch is still real release drift, but it should stay secondary until the remaining mapped A-D/F work is actually coherent enough to freeze. Do not spend watchdog or supervisor runs rephrasing the same six-file mismatch or approval target unless the component-changing boundary, live evidence, or operator instruction materially changes. When formal deploy/restart really is next, ask James directly for approval against `a470bbf`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `a470bbf` build is reviewed in live Home Assistant.
