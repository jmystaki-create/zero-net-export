...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper still resolves the exact component-changing deploy boundary at `8510012`, while `repo_head_commit` is now the later docs-only refresh `184be28`. There has been no newer component-changing repo movement after `8510012`, so the release candidate itself did not move in this run.
- The strongest concrete drift in this run is loop behavior, not a new product gap: with the same six-file live mismatch still unchanged and `184be28` touching only docs, another boundary-refresh narration would be the hash/bookkeeping churn the supervisor explicitly warns against.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary, live evidence, or operator instruction actually changed. When formal deploy/restart really is next, ask James directly for approval against `8510012` instead of rotating more release bookkeeping.
