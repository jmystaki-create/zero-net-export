...
- Recent repo movement also shows a second ranking drift: the latest component-changing commits have been small `top` versus `surfaced` candidate-label neutralization follow-ons across the command center, promotion forms, and review helpers. That copy is already neutral enough to stop being the default repo-side target. Do not keep reopening Workstream C for more label churn unless a stronger operator-path inconsistency appears than the remaining mapped A/B/D/F gaps.
