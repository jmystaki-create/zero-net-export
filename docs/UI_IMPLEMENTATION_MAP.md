...
- This run also rechecked `scripts/print_expected_install_fingerprint.py`, and the exact component-changing deploy anchor has now moved to `21049b5`, not the older `2d730e1`. The repo has moved again for a real native Diagnostics-path fix, while full repo `HEAD` is now `67a6933` because deploy helper hardening landed after that without changing the tracked component fingerprint.
- The release-stage prompting drift is back in the repo steering files: `README.md` and `project_status.md` were still asking for approval on stale build `2d730e1` even though the current exact component candidate is `21049b5`. That stale release boundary is now corrected, but the bigger ranking lesson is that unchanged live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` repo-side runway still remains. Stop spending watchdog/supervisor runs on unchanged fingerprint checks, stale approval-target refreshes, or premature deploy asks before the remaining A-D/F work is coherent enough for Workstream G.
