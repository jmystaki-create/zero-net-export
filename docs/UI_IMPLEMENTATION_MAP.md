...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the same six tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`. Live and repo `manifest_version` both still read `0.1.86`, stale backup artifacts remain absent, and `scripts/print_expected_install_fingerprint.py` still resolves the exact component-changing deploy boundary to `3b92f03`.
- The strongest drift in this run is workstream-order drift, not a new Home Assistant regression and no longer source-of-truth lag: after the boundary correction to `3b92f03`, the latest component-changing repo work is still the Workstream E device-page follow-on in `3b92f03` instead of the higher unfinished Workstreams A-D/F that this map says should lead.
- Keep the ranking lesson intact: unchanged live exact-build mismatch is still real release drift, but it should stay secondary while mapped `0.1.87` repo-side runway still exists. Do not spend watchdog or supervisor runs rephrasing the same six-file mismatch or approval target unless the component-changing boundary, live evidence, or operator instruction materially changes. Likewise, do not reopen device-page deeper-review polish by default unless a real new Workstream E repo-side regression appears. When formal deploy/restart really is next, ask James directly for approval against `3b92f03`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path, promotion flow, and four-bucket IA still do not feel proven until the exact current `3b92f03` build is reviewed in live Home Assistant.
