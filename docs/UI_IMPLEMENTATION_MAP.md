...
- The release-stage prompting drift is back in the repo steering files: `README.md` and `project_status.md` were still asking for approval on stale build `2d730e1` even though the current exact component candidate is `21049b5`. The strongest remaining issue is still the unchanged live exact-build mismatch plus the formal approval boundary, so ask James directly for explicit deploy/restart approval for exact build `21049b5` and stop spending watchdog/supervisor runs on unchanged fingerprint checks or stale approval-target refreshes.
