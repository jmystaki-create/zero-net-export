...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). In this run, the documented SSH path still works and `python3 scripts/print_expected_install_fingerprint.py` now reports `expected_commit=60be5de`, `expected_component_commit=60be5de`, `preferred_validation_commit=60be5de`, and `repo_head_commit=60be5de`. That means any watchdog/supervisor update still framing `390a516` as the exact deploy target is now stale. The live blocker is still one undeployed component build rather than a new product regression, but the exact build to approve/deploy is now `60be5de`.
...
- The next gap is to finish or explicitly retire the highest remaining repo-side UI step in workstream order, then judge exact build `60be5de` in live Home Assistant instead of elevating unchanged validation bookkeeping ahead of product work. A mismatch refresh without new deploy/restart evidence, a newly changed exact-build anchor because repo-side UI work landed, or a newly retired repo-side UI step is churn rather than progress.
