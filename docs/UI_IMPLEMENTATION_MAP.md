...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `bade75f`, not the earlier `e298061`, so the source-of-truth docs must stop describing post-`e298061` repo movement as doc-only churn.
- Repo-side `0.1.87` shaping across Workstreams A-F still reads materially complete enough that default watchdog/supervisor cycles should not keep pretending there is a large untouched repo runway ahead of exact-build deploy/restart approval. But the opposite drift is stale too: the newest repo movement is no longer only release-wording refreshes. `bade75f` is a real Workstream C copy change in shipped component files (`button.py`, `strings.json`, `translations/en.json`), so approval/deploy guidance has to distinguish unchanged live mismatch from genuinely new repo-side UI movement.
- The next real boundary is conditional, not frozen to the old `e298061` story: if the current repo candidate is being presented as release-ready, ask James directly for explicit deploy/restart approval for exact build `bade75f`; otherwise keep working the next mapped `0.1.87` UI step instead of repeating unchanged fingerprint bookkeeping or stale approval-target wording.
