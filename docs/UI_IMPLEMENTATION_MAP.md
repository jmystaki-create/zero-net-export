...
- The next real boundary is exact-build release execution: ask James directly for explicit deploy/restart approval when the current repo candidate is being presented as release-ready, then judge exact build `6cb7062` in live Home Assistant instead of elevating unchanged fingerprint bookkeeping or stale approval wording ahead of that approval boundary.
