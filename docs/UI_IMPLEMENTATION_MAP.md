...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the same six tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`. Live and repo `manifest_version` both still read `0.1.86`, stale backup artifacts remain absent, and `scripts/print_expected_install_fingerprint.py` now resolves the exact component-changing deploy boundary to `c410598`.
- The strongest concrete drift in this run is source-of-truth release-boundary drift again. This file was still naming `594a68a` as the current exact-build target, but the later component-changing follow-on in `native_support.py` moved the helper-resolved boundary to repo `HEAD` `c410598`.
