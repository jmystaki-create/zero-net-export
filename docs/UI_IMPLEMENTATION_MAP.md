...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `ae860ad`, not `4aa43cd`. The change is real repo movement, not doc-only churn: after `4a8792d` refreshed docs to `4aa43cd`, later component-changing work landed again in the same tracked mismatch set, with `8fb03e1`, `ebd9618`, and then `ae860ad` moving `config_flow.py`, `button.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` forward together.
- The strongest concrete drift in this run is stale release-boundary guidance again, not a new live product regression: this file and `docs/BUGS.md` were still steering against `4aa43cd` even though the helper-resolved deploy target has moved to `ae860ad`. The live mismatch itself is otherwise unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping or approval-target refreshes unless the component-changing release boundary, live evidence, or operator instruction actually changed. In this run it did change, so the source-of-truth target needs to move to `ae860ad`. The immediate prelude here matters too: `4a8792d` refreshed docs to `4aa43cd`, then `8fb03e1`, `ebd9618`, and `ae860ad` immediately moved the component target again, which is exactly the loop/churn pattern this file warns against. When formal deploy/restart really is next, ask James directly for approval against `ae860ad`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `ae860ad` build is reviewed in live Home Assistant.
