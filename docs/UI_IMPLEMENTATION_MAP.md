...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `1511450`, not the older `fe6913a` target still named here before. The change is real repo movement, not doc-only churn: after `fe6913a` advanced the same tracked mismatch set, later component-changing work in `1a4816c`, `1523f24`, and `1511450` kept moving `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`, while the live mismatch shape itself stayed the same six files.
- The strongest concrete drift in this run is stale release-boundary guidance again, not a new live product regression: this file and overlapping bug-tracker entries were still steering against `fe6913a` even though the helper-resolved deploy target has moved to `1511450`. The live mismatch itself is otherwise unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping or approval-target refreshes unless the component-changing release boundary, live evidence, or operator instruction actually changed. In this run it did change again, so the source-of-truth target needs to move to `1511450`. The immediate prelude here matters too: `698ef38` and `2e00f48` refreshed docs only, but `1a4816c`, `1523f24`, and `1511450` then kept advancing the same tracked mismatch set without changing the live mismatch shape, which is exactly the loop/churn pattern this file warns against. When formal deploy/restart really is next, ask James directly for approval against `1511450`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `1511450` build is reviewed in live Home Assistant.
