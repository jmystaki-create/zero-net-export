...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper still resolves the exact component-changing deploy boundary at `e33d064`, while `repo_head_commit` has already moved on to the later doc-only `a1d92f4`. The older `63ab6aa` anchor is still stale because later component-changing repo work landed in `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` through `1a83f49` and `e33d064`.
- The strongest concrete drift in this run is source-of-truth churn, not a new live product regression: the last doc-only refresh already moved repo `HEAD`, but this file and `docs/BUGS.md` were still narrating `repo_head_commit` as if it were the same as the component-changing deploy target. The live mismatch itself is unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or mirrored `repo_head_commit` hash edits unless the component-changing release boundary, live evidence, or operator instruction actually changed. When formal deploy/restart really is next, ask James directly for approval against `e33d064` instead of rotating another doc-only head hash.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `e33d064` build is reviewed in live Home Assistant.
