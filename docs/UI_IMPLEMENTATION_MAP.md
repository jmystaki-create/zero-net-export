...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper still resolves the exact current component-changing deploy boundary. Newer component commits after the last previously documented anchor are additional real `0.1.87` UI movement in `button.py`, `candidate_utils.py`, `config_flow.py`, `sensor.py`, `strings.json`, and `translations/en.json`, so source-of-truth text should defer to the helper output instead of hardcoding rotating commit hashes that immediately go stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup, but parts of the docs were still framing the exact-build boundary around hash refreshes instead of the helper-driven boundary. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
