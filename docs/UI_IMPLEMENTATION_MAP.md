...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `7be3ff9`, so the remaining exact-build blocker is the same undeployed managed-device follow-on plus its synced translation-copy drift, not a new install-health regression.
- The strongest concrete drift in this run is stale source-of-truth bookkeeping. `docs/UI_IMPLEMENTATION_MAP.md` and `docs/BUGS.md` had both fallen behind the helper-driven release boundary and were still naming the older `0be21db` / four-file mismatch picture even after `7be3ff9` materially changed `config_flow.py`, `strings.json`, and `translations/en.json`.
