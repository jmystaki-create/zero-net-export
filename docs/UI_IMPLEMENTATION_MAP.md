...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `e33d064`, with `repo_head_commit` also at `e33d064`. The older `63ab6aa` anchor is now stale because later component-changing repo work landed in `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json` through `1a83f49` and `e33d064`.
- The strongest concrete drift in this run is stale source-of-truth release guidance again, not a new live product regression: `docs/UI_IMPLEMENTATION_MAP.md` and `docs/BUGS.md` were still telling watchdog/supervisor flows to treat `63ab6aa` as the current approval/deploy target even though the helper-driven exact-build boundary moved to `e33d064` while the same six-file live mismatch stayed unchanged.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary, live evidence, or operator instruction actually changed. When formal deploy/restart really is next, ask James directly for approval against `e33d064` instead of the now-stale `63ab6aa` target.
