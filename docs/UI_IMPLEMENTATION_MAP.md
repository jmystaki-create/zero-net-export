...
- Repo-side `0.1.87` shaping across Workstreams A-F still reads materially complete enough that default watchdog/supervisor cycles should not keep pretending there is a large untouched repo runway ahead of exact-build deploy/restart approval. `bade75f` remains the latest component-changing candidate, and the later `069dc68` and `c141ddf` commits are docs-only source-of-truth refreshes around the same unchanged live mismatch, not new shipped UI movement.
- The next real boundary is still conditional: if the current repo candidate is being presented as release-ready, ask James directly for explicit deploy/restart approval for exact build `bade75f`; otherwise keep working the next mapped `0.1.87` UI step. Do not keep spending watchdog/supervisor runs on unchanged fingerprint bookkeeping, approval-target retargeting, or doc-only refreshes that do not change the actual release decision.
