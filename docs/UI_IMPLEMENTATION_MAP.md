...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, now with two tracked-file mismatches, `native_support.py` and `sensor.py`. Live and repo `manifest_version` both read `0.1.86`, and the earlier stale backup bytecode artifact is still absent, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `8eb8abb`, so the remaining exact-build blocker is the current two-file drift on the latest repo-side Managed Devices handoff copy rather than the older twelve-file mixed-install state.
- The strongest concrete drift in this run is source-of-truth lag again: these docs had already been corrected from the older twelve-file `0.1.87` mismatch story, but they were still understating the current live drift as one-file `native_support.py` only even after the latest component-changing repo fix also moved `sensor.py`.
