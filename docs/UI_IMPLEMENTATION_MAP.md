...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `5f89845`, not `ce05b95`. The change is real repo movement, not doc-only churn: later component-changing work landed again in the same tracked mismatch set, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`.
- The strongest concrete drift in this run is stale release-boundary guidance again, not a new live product regression: this file and `docs/BUGS.md` were still steering against `ce05b95` even though the helper-resolved deploy target has moved to `5f89845`. The live mismatch itself is otherwise unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping or approval-target refreshes unless the component-changing release boundary, live evidence, or operator instruction actually changed. In this run it did change, so the source-of-truth target needs to move to `5f89845`. The immediate prelude here matters too: `52b2630` was another doc-only boundary refresh and `5f89845` immediately moved the component target again, which is exactly the loop/churn pattern this file warns against. When formal deploy/restart really is next, ask James directly for approval against `5f89845`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `5f89845` build is reviewed in live Home Assistant.
