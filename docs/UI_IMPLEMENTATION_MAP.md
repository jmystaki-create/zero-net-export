...
- The stale ranking drift in this run is now in the source-of-truth narrative itself: it was still framing post-`ca67d61` repo movement as mostly Workstream C candidate-label churn even though `31e8125` and `e02c34d` are newer component-changing follow-ons in Workstreams F and B. Repo `HEAD` is no longer the doc-only `6504e49` state named in older notes, and the strongest current lesson is not to keep narrating the repo as if candidate-label neutralization were still the default target. Keep the ranking on the highest remaining mapped A/B/D/F gap plus live-proof needs, and do not reopen Workstream C unless a fresher operator-path inconsistency is stronger than those remaining mapped gaps.
