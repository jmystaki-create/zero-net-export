...
- This watchdog run's documented HA SSH fingerprint recheck now reports `overall_match=false`; live Home Assistant is drifted again (`manifest_version=0.1.87` versus repo `0.1.86`, plus `manifest.json`, `native_support.py`, and `sensor.py` mismatches), so repo-versus-live exact-build drift is again the active release blocker until James explicitly approves redeploy/restart.
