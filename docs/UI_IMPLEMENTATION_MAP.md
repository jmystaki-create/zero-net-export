...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `e316257`, not the earlier `47e157f`, because a fresh repo-side managed-summary fix landed in `sensor.py`.
- That new component change means the source-of-truth docs should stop framing exact-build approval for `47e157f` as the default next boundary. `e316257` is a real Workstream B/E follow-on that strengthens managed-device row density by surfacing nominal power in the primary managed summary, so release-stage wording should not overstate unchanged live validation as the main next gap while that repo-side shaping is still moving.
- The next real boundary is now: validate whether the current managed-summary follow-on leaves any remaining repo-side Managed Devices workspace drift worth fixing before release freeze; only after that should the project ask James directly for explicit deploy/restart approval for exact build `e316257`. Do not keep spending watchdog/supervisor runs on unchanged fingerprint bookkeeping or stale approval-target retargeting.
