...
- This run also rechecked `scripts/print_expected_install_fingerprint.py`, and the exact component-changing deploy anchor is still `2d730e1`, not `78396f3`. `78396f3` is only a docs refresh of the same unchanged release-drift state, so it is not new release evidence and should not become a fresh approval target.
- The release-stage prompting drift was already corrected before this run. The strongest remaining issue is the same unchanged live exact-build mismatch plus the formal approval boundary: ask James directly for explicit deploy/restart approval for exact build `2d730e1`, and do not spend more watchdog/supervisor runs on unchanged fingerprint checks or docs-head refreshes.
