...
- This run rechecked the documented HA SSH path and the mismatch is still unchanged: `overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set (`button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper still resolves the exact current component-changing deploy boundary. Treat that helper output, not a hard-coded commit hash in these source-of-truth docs, as the release anchor.
- The stronger drift in this run is still release-anchor churn, not a new live-fingerprint result: the documented HA SSH check still returns the same unchanged eleven-file mismatch plus the same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact. Keep the ranking lesson intact: unchanged live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` repo-side runway still remains. Stop spending watchdog/supervisor runs on unchanged fingerprint checks, stale approval-target refreshes, or hash-only source-of-truth edits before the remaining A-D/F work is coherent enough for Workstream G.
- Recent repo movement also shows a second ranking drift: the latest component-changing commits have been small `top` versus `surfaced` candidate-label neutralization follow-ons across the command center, promotion forms, and review helpers. That copy is already neutral enough to stop being the default repo-side target. The current `HEAD` commit `6504e49` is doc-only, and `scripts/print_expected_install_fingerprint.py` still resolves the same component-changing deploy boundary at `ca67d61`, so there is no new product-side reason to reopen Workstream C or rerun unchanged release bookkeeping here. Do not keep reopening Workstream C for more label churn unless a stronger operator-path inconsistency appears than the remaining mapped A/B/D/F gaps.
