...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `63ab6aa`, with `repo_head_commit` also at `63ab6aa`. The older `9a03974` anchor is now stale because later component-changing repo work landed in `config_flow.py` and the managed-device per-kind backlog split.
- The strongest concrete drift in this run is now stale source-of-truth release guidance, not another unchanged fingerprint loop: `docs/UI_IMPLEMENTATION_MAP.md` and `docs/BUGS.md` were still telling watchdog/supervisor flows to treat `9a03974` as the current approval/deploy target even though the helper-driven exact-build boundary moved to `63ab6aa`.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary, live evidence, or operator instruction actually changed. When formal deploy/restart really is next, ask James directly for approval against `63ab6aa` instead of the stale `9a03974` target.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `63ab6aa` build is reviewed in live Home Assistant.
