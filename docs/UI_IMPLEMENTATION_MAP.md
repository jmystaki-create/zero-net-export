...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `e880af9`, not the earlier `40cb55e` checkpoint. The live mismatch evidence itself is unchanged, but the repo candidate moved again because `42869cc`, `5090d9e`, and then `e880af9` landed as real Workstream D cleanup after the older boundary notes.
- The strongest concrete drift in this run is source-of-truth lag again: the implementation map was still steering off `40cb55e` even after `e880af9` kept tightening the command-center Controls handoff under truncation. That is exactly the kind of stale release-boundary bookkeeping churn the supervisor warns against, because it keeps older candidate framing in front of still-moving `0.1.87` repo runway.
...
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `e880af9` build is reviewed in live Home Assistant.
