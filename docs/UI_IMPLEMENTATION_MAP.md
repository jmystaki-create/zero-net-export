...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `78c5023`, not the earlier `e316257`, because a fresh repo-side command-center follow-on landed in `native_support.py`.
- That new component change means the source-of-truth docs should stop framing exact-build approval for `e316257` as the default next boundary. `78c5023` is a real Workstream A/B follow-on that keeps the command-center device-status line explicit about mixed unmanaged backlog (`unmanaged`, `needs review`, `ready to promote`) instead of collapsing back to the misleading `unmanaged ready` shorthand, including the managed-device-config-repair path.
- The next real boundary is now: validate whether this command-center mixed-backlog follow-on leaves any remaining repo-side operator-console or Managed Devices wording drift worth fixing before release freeze; only after that should the project ask James directly for explicit deploy/restart approval for exact build `78c5023`. Do not keep spending watchdog/supervisor runs on unchanged fingerprint bookkeeping or stale approval-target retargeting.
