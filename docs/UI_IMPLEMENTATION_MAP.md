...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. Since that last live fingerprint check, repo work moved again in a component-changing way twice: `08f344e` tightened install-provenance guidance so native release-repair text now asks James explicitly for deploy/restart approval before showing deploy commands, and later `e298061` hardened the opening command-center overflow fallback so fleet review cues no longer collapse away under the 255-character state limit. The release blocker is still exact-build drift, but the deploy target is now `e298061`, not the older `08f344e` anchor.
- Repo-side `0.1.87` shaping across Workstreams A-F still reads materially complete enough that default watchdog/supervisor cycles should not keep pretending there is a large untouched repo runway ahead of exact-build deploy/restart approval. This run also confirms that claim should stay edge-triggered, not absolute: a fresh Workstream A drift did appear, it was safe to ship, and it moved the real candidate to `e298061`.
- The next real boundary is exact-build release execution: ask James directly for explicit deploy/restart approval when the current repo candidate is being presented as release-ready, then judge exact build `e298061` in live Home Assistant instead of elevating unchanged fingerprint bookkeeping or stale approval wording ahead of that approval boundary.
