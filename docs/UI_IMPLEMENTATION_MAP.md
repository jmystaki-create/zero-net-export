...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `33da862`, not the older `3cb7b36` checkpoint. The live mismatch evidence itself is unchanged, but the repo candidate moved again because `25f2b70` and `33da862` landed after the last docs refresh and both still sit inside active Workstream A/B cleanup.
- The strongest concrete drift in this run is source-of-truth lag again: the implementation map was still steering off the earlier `3cb7b36` boundary even after `33da862` preserved the managed/unmanaged fleet-activity story under summary compaction. That is exactly the kind of stale release-boundary bookkeeping churn the supervisor warns against, because it keeps older candidate framing in front of still-moving `0.1.87` repo runway.
...
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `33da862` build is reviewed in live Home Assistant.
