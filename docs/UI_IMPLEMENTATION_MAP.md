...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, and the release drift remains twelve tracked-file mismatches (`__init__.py`, `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact is still present, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `316aa5b`. The intervening docs-only commits `4f5725c` and `9d373ca` were bookkeeping-only, but the newer component commits `cc91e58`, `edb8281`, and `316aa5b` are additional real `0.1.87` UI movement in `button.py`, `candidate_utils.py`, `config_flow.py`, `sensor.py`, `strings.json`, and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `f94cd7f`, `facc3af`, or `732050f` is stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup through `316aa5b`, but parts of the docs were still framing the exact-build boundary around older anchors. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
- Keep the ranking lesson intact: live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` runway still remains. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary or operator instruction actually changed.
