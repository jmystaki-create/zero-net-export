...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is not unchanged: `overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, and the release drift remains twelve tracked-file mismatches (`__init__.py`, `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact is still present, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `0a83406`. The earlier `129a5a7`, `b0936f0`, and `516e75f` docs updates were bookkeeping-only, but `0a83406` is a real Workstream D follow-on in `strings.json` and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `6d993f6` is now stale again.
- The strongest concrete drift in this run is still watchdog/supervisor loop behavior: once the source-of-truth docs had already been corrected to stop stale `e02c34d` bookkeeping, the next repo movement shifted back into real product work at `0a83406`, but the docs themselves were still framing the release boundary as `6d993f6`. That is source-of-truth drift, not a new reason to elevate Workstream G over unfinished mapped A/B/D/F runway.
- Keep the ranking lesson intact: live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` A/B/D/F runway still remains. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits unless the release boundary or operator instruction actually changed.
