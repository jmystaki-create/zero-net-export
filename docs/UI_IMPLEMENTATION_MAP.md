...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, now with four tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, and `sensor.py`. Live and repo `manifest_version` both read `0.1.86`, and the earlier stale backup bytecode artifact is still absent, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `31df216`, so the remaining exact-build blocker is the current four-file drift on the latest empty-fleet Managed Devices handoff follow-ons rather than the older twelve-file mixed-install state.
- The strongest concrete drift in this run is no longer stale mismatch wording. After the same four-file exact-build blocker was already documented, recent commits `e11ee6f`, `e0ee570`, `9a2361a`, `47380a2`, and `31df216` still spent five more runs on empty-fleet handoff micro-follow-ons instead of advancing the next unfinished `0.1.87` workstream or explicitly asking James for release approval if deploy/restart had actually become the boundary.
