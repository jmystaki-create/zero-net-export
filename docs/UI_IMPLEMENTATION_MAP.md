...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `8510012`, not the earlier `e880af9` checkpoint. The live mismatch evidence itself is unchanged, but the repo candidate moved again because the ready-only command-center handoff fix in `8510012` is another real Workstream A/B product change on one of the still-mismatched live files, `native_support.py`.
- The strongest concrete drift in this run is source-of-truth lag again: the implementation map was still steering off `e880af9` even after `8510012` tightened the opening command-center handoff for ready-only unmanaged backlog. That is exactly the kind of stale release-boundary bookkeeping churn the supervisor warns against, because it keeps yesterday's candidate framing in front of still-moving `0.1.87` repo runway.
...
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `8510012` build is reviewed in live Home Assistant.
