...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `2d730e1`, not `840695e`, because a fresh repo-side Workstream F follow-on landed in `strings.json` and `translations/en.json`.
- That new component change means the source-of-truth docs should stop framing exact-build approval for `840695e` as the default next boundary. `2d730e1` is a real diagnostics-triage follow-on that keeps the Configure Diagnostics step compact and blocker-first instead of regressing back into a longer source-map/support wall.
- This run's repo audit did not find a fresh mapped Workstream A-F repo-side gap. The release-stage prompting drift was corrected in the latest docs/status follow-on, so the strongest remaining issue is now the same unchanged live exact-build mismatch plus the formal approval boundary: ask James directly for explicit deploy/restart approval for exact build `2d730e1`, and do not spend more watchdog/supervisor runs on unchanged fingerprint bookkeeping.
