...
- Repeated doc-only release-boundary refresh commits are process drift, not product progress. The helper-resolved component boundary is now `814ff00` because a real component change landed after the earlier `93c383a` freeze; later docs-only commits still do not create a new release target and should not displace the mapped Workstream A-F gap or a plain no-change report.
