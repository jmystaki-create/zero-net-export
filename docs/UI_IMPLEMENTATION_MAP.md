...
- Repeated doc-only release-boundary refresh commits are process drift, not product progress. The helper-resolved component boundary is now `8044a21` because a real component fix landed after the earlier `814ff00` boundary; later docs-only commits still do not create a new release target and should not displace the mapped Workstream A-F gap or a plain no-change report.
