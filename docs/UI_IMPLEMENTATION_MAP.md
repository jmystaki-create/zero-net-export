...
- The next gap is to finish or explicitly retire the highest remaining repo-side UI step in workstream order, then judge exact build `481c9a7` in live Home Assistant instead of elevating unchanged validation bookkeeping ahead of product work. A mismatch refresh without new deploy/restart evidence is still churn, but so is claiming the deploy target is unchanged when a newer component build exists.
