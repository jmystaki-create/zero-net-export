...
- The live Home Assistant install still does not match the current repo candidate. This run's SSH fingerprint recheck now shows 23 tracked-file mismatches (`__init__.py`, `binary_sensor.py`, `button.py`, `candidate_utils.py`, `config_flow.py`, `const.py`, `coordinator.py`, `device_model.py`, `diagnostics.py`, `entity.py`, `executor.py`, `manifest.json`, `native_support.py`, `number.py`, `planner.py`, `release_info.py`, `repairs.py`, `select.py`, `sensor.py`, `strings.json`, `switch.py`, `translations/en.json`, `validation.py`), so exact-build redeploy plus fingerprint revalidation is still required before final release judgment or screenshot-grade acceptance proof.
