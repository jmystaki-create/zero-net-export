...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `ec59165`, not `6b730a1`. The change is real repo movement, not doc-only churn: after `4de55b9` refreshed docs to `6b730a1`, later component-changing work landed again in the same tracked mismatch set, with `5e9e7c6`, `95592b6`, and `ec59165` moving `sensor.py`, `config_flow.py`, and then `native_support.py` plus `sensor.py` forward together while the live mismatch set itself stayed the same six files.
- The strongest concrete drift in this run is stale release-boundary guidance again, not a new live product regression: this file and overlapping bug-tracker entries were still steering against `6b730a1` even though the helper-resolved deploy target has moved to `ec59165`. The live mismatch itself is otherwise unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping or approval-target refreshes unless the component-changing release boundary, live evidence, or operator instruction actually changed. In this run it did change, so the source-of-truth target needs to move to `ec59165`. The immediate prelude here matters too: `4de55b9` refreshed docs to `6b730a1`, then `5e9e7c6`, `95592b6`, and `ec59165` immediately moved the component target again, which is exactly the loop/churn pattern this file warns against. When formal deploy/restart really is next, ask James directly for approval against `ec59165`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `ec59165` build is reviewed in live Home Assistant.
