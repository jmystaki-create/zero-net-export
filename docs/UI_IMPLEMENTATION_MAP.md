...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with six tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`. Live and repo `manifest_version` both still read `0.1.86`, and stale backup artifacts remain absent, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper still resolves the exact component-changing deploy boundary at `14c06e6`, so the remaining exact-build blocker is still the same undeployed Managed Devices follow-on chain, now including the later workspace-wording and managed-runtime-duration updates, not a new install-health regression.
- The strongest concrete drift in this run is repeated source-of-truth bookkeeping after the docs were already refreshed to `14c06e6`. `057a8b0` changed only `docs/UI_IMPLEMENTATION_MAP.md` and `docs/BUGS.md`, but the live mismatch set stayed the same six files and no new release evidence appeared, so another boundary-refresh pass would be churn rather than progress.
