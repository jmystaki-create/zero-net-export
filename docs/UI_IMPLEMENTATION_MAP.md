...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, and the release drift still spans the same twelve tracked-file mismatches (`__init__.py`, `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact is still present, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `edb8281`. The immediately preceding commit `cc91e58` and current `edb8281` are both real Managed Devices / promotion-flow component changes in `button.py`, `candidate_utils.py`, `config_flow.py`, `sensor.py`, `strings.json`, and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `f94cd7f`, `facc3af`, or older hashes is stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup through `edb8281`, but the live mismatch class is unchanged. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
