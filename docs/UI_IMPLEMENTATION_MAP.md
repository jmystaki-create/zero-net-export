...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `5a9fd29`, which means the current exact-build release blocker is still real but the older `6bbcc5b` / `973d1f7` release-boundary story is now stale source-of-truth state.
- The strongest concrete drift in this run is now source-of-truth lag, not another new release-boundary churn cycle: repo HEAD has advanced through real Managed Devices promotion-flow fixes (`fe0e7b0`, `5a9fd29`), but the implementation map still described the last meaningful boundary as the earlier docs-only `973d1f7` checkpoint after `17246ba` and `6bbcc5b`.
