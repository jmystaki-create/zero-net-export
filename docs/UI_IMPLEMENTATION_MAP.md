...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `facc3af`. The later docs-only commits `099640d`, `c05c697`, and `aef9a2c` were bookkeeping-only, but the more recent `fa182d4` and `facc3af` commits are additional real `0.1.87` UI movement in `sensor.py` and `native_support.py`, so any source-of-truth text still anchoring release discussion on `77f43be` or `732050f` is stale again.
- The strongest concrete drift in this run is still source-of-truth lag, not a new release-stage fact: the repo has moved forward with real Managed Devices / promotion follow-through cleanup through `facc3af`, but parts of the docs were still framing the exact-build boundary around older anchors. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
