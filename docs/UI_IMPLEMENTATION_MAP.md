...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper now resolves the exact current component-changing deploy boundary as `732050f`. The older `129a5a7`, `b0936f0`, and `516e75f` docs updates were bookkeeping-only, but the later `0a83406`, `a03f80f`, `894d211`, `408d7a1`, and `732050f` commits are real Workstream D code movement in `config_flow.py`, `strings.json`, and `translations/en.json`, so any source-of-truth text still anchoring release discussion on `0a83406` is stale again.
- The strongest concrete drift in this run is source-of-truth lag, not a new release-stage fact: the repo has moved forward with real four-bucket IA work through `732050f`, but parts of the docs were still framing the exact-build boundary around older anchors. That is doc drift, not a reason to elevate Workstream G over unfinished mapped A/B/C/D/F runway.
