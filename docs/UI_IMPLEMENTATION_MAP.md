...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `fe6913a`, not `703ae4b`. The change is real repo movement, not doc-only churn: after `703ae4b` moved `config_flow.py` forward, later component-changing work in `eb3e2e2` and `fe6913a` kept moving the same tracked mismatch set through `button.py`, `config_flow.py`, `native_support.py`, and `sensor.py`, while the live mismatch set itself stayed the same six files.
- The strongest concrete drift in this run is stale release-boundary guidance again, not a new live product regression: this file and overlapping bug-tracker entries were still steering against `703ae4b` even though the helper-resolved deploy target has moved to `fe6913a`. The live mismatch itself is otherwise unchanged, still the same six tracked files.
- Keep the ranking lesson intact: live exact-build mismatch is still release drift, but it should not be promoted over unfinished mapped repo work. Do not spend watchdog/supervisor runs on repeated fingerprint bookkeeping or approval-target refreshes unless the component-changing release boundary, live evidence, or operator instruction actually changed. In this run it did change, so the source-of-truth target needs to move to `fe6913a`. The immediate prelude here matters too: `2f68a30` refreshed docs to `ec59165`, `703ae4b` immediately moved the component target again, and later `eb3e2e2` plus `fe6913a` advanced the same tracked files without changing the live mismatch shape, which is exactly the loop/churn pattern this file warns against. When formal deploy/restart really is next, ask James directly for approval against `fe6913a`.
- Live runtime stability still needs to be strong enough that the UI can be judged honestly in Home Assistant.
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `fe6913a` build is reviewed in live Home Assistant.
