...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the same six tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, `sensor.py`, `strings.json`, and `translations/en.json`. Live and repo `manifest_version` both still read `0.1.86`, stale backup artifacts remain absent, and `scripts/print_expected_install_fingerprint.py` still keeps the exact component-changing deploy boundary at `1511450` even though repo `HEAD` has since advanced through docs-only commits.
- The strongest concrete drift in this run is source-of-truth ranking drift, not a newly changed release boundary: this file was still foregrounding the previous run's `fe6913a` to `1511450` approval-target refresh even though today's helper output and live mismatch shape are unchanged. Repeating that same release-anchor bookkeeping again would be loop churn, and it incorrectly elevates unchanged live validation over the still-unfinished mapped `0.1.87` repo runway.
- Keep the ranking lesson intact: live exact-build mismatch is still real release drift, but it should stay secondary until the remaining mapped A-D/F work is actually coherent enough to freeze. Do not spend watchdog or supervisor runs rephrasing the same six-file mismatch, candidate hash, or approval target unless the component-changing boundary, live evidence, or operator instruction materially changes. When formal deploy/restart really is next, ask James directly for approval against `1511450`.
