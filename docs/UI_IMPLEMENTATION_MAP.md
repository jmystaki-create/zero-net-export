...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper still resolves the exact component-changing deploy boundary at `6bbcc5b`, and the documented HA SSH path still confirms the same unchanged twelve-file live drift plus stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact. The release blocker did not materially change in this run.
- The strongest concrete drift in this run was loop churn in source-of-truth maintenance: `git log --oneline -12` shows repo HEAD moved to docs-only `973d1f7 docs: realign release boundary audit` after the last real component changes `17246ba` and `6bbcc5b`, but there was no new deploy, restart, screenshot, or other live evidence to justify another release-boundary refresh.
