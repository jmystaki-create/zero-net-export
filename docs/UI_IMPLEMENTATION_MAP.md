...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the same four tracked-file mismatches, `button.py`, `config_flow.py`, `native_support.py`, and `sensor.py`. Live and repo `manifest_version` both still read `0.1.86`, and stale backup artifacts remain absent, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `0be21db`, so the remaining exact-build blocker is still the same four-file drift on `button.py`, `config_flow.py`, `native_support.py`, and `sensor.py` rather than a new install-health regression.
- The strongest concrete drift in this run is supervisor/bookkeeping churn, not a new product blocker. After the same four-file exact-build mismatch was already documented, one more docs-only drift refresh (`7c50671`) landed without new live evidence, while the release boundary and the remaining `0.1.87` workstream order stayed unchanged.
