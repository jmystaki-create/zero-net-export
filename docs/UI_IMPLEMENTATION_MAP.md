...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is still not the current repo candidate: `overall_match=false`, with the mismatch now narrowed to `native_support.py` only. Live and repo `manifest_version` both read `0.1.86`, and the earlier stale backup bytecode artifact is no longer present, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `32d802b`, so the remaining exact-build blocker is a one-file live drift on the current repo-side command-center/Managed Devices copy rather than the older multi-file mixed-install state.
- The strongest concrete drift in this run is stale release-state wording: source-of-truth docs were still describing a twelve-file `0.1.87` mismatch even though the live SSH comparison now shows one-file `native_support.py` drift on `0.1.86`. Keep treating that as release drift, but do not let the old mismatch story replace the higher-ranked remaining `0.1.87` implementation runway.
