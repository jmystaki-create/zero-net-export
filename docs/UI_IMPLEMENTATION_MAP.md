...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. Since that last live fingerprint check, repo work has moved again in a component-changing way: `481c9a7` updates `sensor.zero_net_export_managed_fleet_overview` so the managed workspace keeps the explicit ready-to-promote backlog count plus matching review/ready candidate attributes visible. That means the release blocker is still exact-build drift, but the deploy target is no longer the older `7dfb76d` anchor.
- Repo-side `0.1.87` shaping across Workstreams A-F now reads materially complete enough that more default watchdog/supervisor cycles should not keep pretending there is still a larger unfinished repo runway ahead of exact-build deploy/restart approval. Unless a fresh repo-side drift appears, those workstreams are now in live-proof posture.
- The next real boundary is exact-build release execution: ask James directly for explicit deploy/restart approval when the current repo candidate is being presented as release-ready, then judge exact build `481c9a7` in live Home Assistant instead of elevating unchanged fingerprint bookkeeping or doc-only anchor refreshes ahead of that approval boundary.
