...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `40cb55e`, not the earlier `46585f7` checkpoint. The live mismatch evidence itself is unchanged, but the repo candidate moved again because `fabdc29`, `e160892`, `566eb66`, and then `40cb55e` landed as real Workstream A/B cleanup after the older boundary notes.
- The strongest concrete drift in this run is source-of-truth lag again: the implementation map was still steering off `46585f7` even after `40cb55e` kept tightening the opening command-center fleet story. That is exactly the kind of stale release-boundary bookkeeping churn the supervisor warns against, because it keeps older candidate framing in front of still-moving `0.1.87` repo runway.
...
- The native Managed Devices path and four-bucket IA still do not feel proven until the exact current `40cb55e` build is reviewed in live Home Assistant.
