...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the exact component-changing deploy anchor has moved again to `113bc33`. Unlike the earlier deploy-helper/bookkeeping-only movement, this one is a real `config_flow.py` shortlist fix, so the source-of-truth release boundary needed to move with it.
- The stronger drift in this run was source-of-truth steering churn: this map and the active release bug state were still naming the older `21049b5` / `67a6933` boundary even though the helper now resolves the current exact component candidate to `113bc33`. Correct that stale anchor, but keep the bigger ranking lesson intact: unchanged live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` repo-side runway still remains. Stop spending watchdog/supervisor runs on unchanged fingerprint checks, stale approval-target refreshes, or premature deploy asks before the remaining A-D/F work is coherent enough for Workstream G.
