...
- This run rechecked the documented HA SSH path and confirmed access still works, but the live install is not unchanged: `overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, and the release drift has widened to twelve tracked-file mismatches (`__init__.py`, `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact is still present, so access is not the blocker.
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the helper still resolves the exact current component-changing deploy boundary as `e02c34d` while repo `HEAD` has moved on to doc-only `6af6be8`. Treat that helper output, not a hard-coded commit hash in these source-of-truth docs, as the release anchor.
- The stronger drift in this run is supervisor/source-of-truth churn, not a new repo-side product blocker: recent repo `HEAD` movement is still documentation-only (`a04b4cd`, `6af6be8`) while unfinished A/B/D/F runway remains, and the latest live SSH check now shows materially different release drift rather than the previously repeated eleven-file result. Keep the ranking lesson intact: live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` repo-side runway still remains. Stop spending watchdog/supervisor runs on repeated fingerprint bookkeeping, approval-target refreshes, or hash-only source-of-truth edits before the remaining A-D/F work is coherent enough for Workstream G.
- The stale ranking drift in this run is now twofold in the source-of-truth narrative itself: it was still framing recent repo movement as mostly Workstream C candidate-label churn even though `31e8125` and `e02c34d` remain the latest component-changing follow-ons in Workstreams F and B, and it was still describing live validation as the same eleven-file mismatch even after the documented HA SSH check widened to twelve mismatches. Keep the ranking on the highest remaining mapped A/B/D/F gap plus live-proof needs, and do not reopen Workstream C unless a fresher operator-path inconsistency is stronger than those remaining mapped gaps.
