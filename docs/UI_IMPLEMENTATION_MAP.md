...
- This run rechecked `scripts/print_expected_install_fingerprint.py`; the helper now resolves the exact component-changing deploy boundary at `14c06e6`, so the remaining exact-build blocker is the same undeployed Managed Devices follow-on chain, now including the later workspace-wording and managed-runtime-duration updates, not a new install-health regression.
- The strongest concrete drift in this run is stale source-of-truth bookkeeping again. `docs/UI_IMPLEMENTATION_MAP.md` and `docs/BUGS.md` were still naming the older `7be3ff9` boundary even after `8bd921d` and `14c06e6` materially changed `button.py` and `config_flow.py` while the live mismatch set stayed otherwise unchanged.
