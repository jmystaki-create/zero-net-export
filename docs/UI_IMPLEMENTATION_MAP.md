...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. In this run `scripts/print_expected_install_fingerprint.py` resolves the current component-changing candidate to `840695e`, not `78c5023`, because a fresh repo-side Workstream C follow-on landed in `config_flow.py`.
- That new component change means the source-of-truth docs should stop framing exact-build approval for `78c5023` as the default next boundary. `840695e` is a real promotion-flow follow-on that keeps the shortlist recap aligned with the design requirement to show candidate name, type, usefulness, and key warning instead of regressing candidate type back out of the first native shortlist summary.
- The next real boundary is now: judge whether this shortlist-summary follow-on leaves any remaining repo-side Workstream C wording drift worth fixing before release freeze; only after that should the project ask James directly for explicit deploy/restart approval for exact build `840695e`. Do not keep spending watchdog/supervisor runs on unchanged fingerprint bookkeeping or stale approval-target retargeting.
