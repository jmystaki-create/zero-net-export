...
- The latest documented HA SSH fingerprint check still shows the same known exact-build mismatch (`overall_match=false`; live `manifest_version=0.1.87` versus repo `0.1.86`, with the same eleven-file drift set: `button.py`, `candidate_utils.py`, `config_flow.py`, `diagnostics.py`, `manifest.json`, `native_support.py`, `release_info.py`, `repairs.py`, `sensor.py`, `strings.json`, and `translations/en.json`). The documented SSH path still works, so access is not the blocker. Since that last live fingerprint check, repo work has moved again in a component-changing way: `a61ea30` separates the Controls bucket path from the live-mode shortcut wording so the four-bucket jump-off copy no longer blurs section ownership. That means the release blocker is still exact-build drift, but the deploy target is now `a61ea30`, not the older `5070a7c` anchor.
...
- The next real boundary is exact-build release execution: ask James directly for explicit deploy/restart approval when the current repo candidate is being presented as release-ready, then judge exact build `a61ea30` in live Home Assistant instead of elevating unchanged fingerprint bookkeeping or stale approval wording ahead of that approval boundary.
