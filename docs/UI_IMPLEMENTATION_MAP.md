...
- This run rechecked `scripts/print_expected_install_fingerprint.py`, and the exact component-changing deploy anchor has moved again to `f8258a7`. The intermediate `976e3f6` docs refresh is not the shipped boundary, but the newer `f8258a7` commit is a real component change in `strings.json` and `translations/en.json`, so `72e9280` is no longer the current release boundary.
- The stronger drift in this run is still source-of-truth steering churn, not a new live-fingerprint result: the documented HA SSH check still returns the same unchanged eleven-file mismatch plus the same stale `/homeassistant/custom_components/zero_net_export/__pycache__/backup.cpython-311.pyc` artifact, but this map and the active release bug were still naming `72e9280` as current after `f8258a7` landed. Correct that stale anchor, and keep the bigger ranking lesson intact: unchanged live exact-build mismatch is release drift, not the default build-stage next step while mapped `0.1.87` repo-side runway still remains. Stop spending watchdog/supervisor runs on unchanged fingerprint checks, stale approval-target refreshes, or premature deploy asks before the remaining A-D/F work is coherent enough for Workstream G.
