...
- make Configure, Repairs, and the native integration device path (`Settings -> Devices & Services -> Integrations -> Zero Net Export -> Devices -> open the Zero Net Export device`) more self-explanatory as the command center for Sensors/source roles, Controls, Managed Devices, and Diagnostics
