...
The repo later froze `v0.1.94` at `4c0d071` after the earlier `v0.1.92` and `v0.1.93` freezes, while this steering file and the release plan still approve only the `0.1.91` / release `1.91` integration-page device-list scope. Until James explicitly decides whether `v0.1.94` replaces the documented `0.1.91` target or the project returns to the approved `0.1.91` boundary, do not deploy, restart, fingerprint-validate, or call successful `7217f3b`, `c4802a3`, `db5c246`, `026f189`, or `4c0d071`. Ask James directly for that release-target decision, then for native-row acceptance and exact deploy/restart approval before Home Assistant actions.
