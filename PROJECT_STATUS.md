...
status: active
last_modified: 2026-05-01 12:05

## Current focus
ZNE-583 is reopened after Riley's screenshot showed `Invalid flow specified` still visible in Home Assistant `0.1.99`; the previous validation only checked that the form content appeared and missed that the returned config-flow payload had `flow_id`/`handler` set to `null`. Repo fix `2b1b73e` preserves the active config-flow identity when delegating to the service options flow and is pushed to `origin/main`. Do not deploy, restart Home Assistant, tag, publish, or claim a replacement release is ready until it is released through HACS, restarted, and revalidated on the live UI.
...
- ZNE-583 — service-row `Reconfigure` opens with `Invalid flow specified`. Reopened 2026-05-01: Riley screenshot from `0.1.99` proves the invalid-flow banner is still visible. Root cause found: the config-flow wrapper delegated to an options-flow form whose `flow_id` and `handler` remained `null`, so Home Assistant could render fields while still reporting an invalid flow. Repo fix committed/pushed as `2b1b73e`; release/HACS/restart/live revalidation still pending. Status: fixed_pending_release_validation.
...
- ZNE-583: previous validation is superseded by Riley's 2026-05-01 screenshot. It proved the validation was incomplete: it checked DOM text after clicking `Reconfigure` but did not assert the raw Home Assistant config-flow identity. New regression coverage now asserts the reconfigure wrapper preserves non-null `flow_id` and `handler` while keeping the first step id as `reconfigure` and the submit transition as `configure_service_sources`. Repo validation passed with `python3 -m py_compile custom_components/zero_net_export/config_flow.py`, focused tests `python3 -m unittest -q tests.test_config_flow_device_runtime_overlay tests.test_operator_docs_consistency tests.test_translation_sync tests.test_bug_tracker_ids` (103 tests OK), full test discovery `python3 -m unittest discover -s tests` (600 tests OK), and `git diff --check`. Fix committed/pushed as `2b1b73e`; release/HACS/restart/live evidence is pending.
...
## Next best action
Release ZNE-583 fix `2b1b73e` through the approved release-management path, install via HACS, restart Home Assistant, then revalidate the exact UI path that showed `Invalid flow specified`.
