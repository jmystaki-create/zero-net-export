...
**Current highest-value next step:** stop treating the stale `e316257` approval target as current. `scripts/print_expected_install_fingerprint.py` now resolves the latest component-changing candidate to exact build `78c5023`, which is a real command-center follow-on after `e316257`, not docs-only churn. The next repo-side question is whether that mixed-backlog wording follow-on leaves any remaining mapped operator-console or Managed Devices drift worth fixing before freeze. Only after that cut line is coherent should James be asked directly for deploy/restart approval on exact build `78c5023`, rather than spending another run on unchanged fingerprint bookkeeping.
