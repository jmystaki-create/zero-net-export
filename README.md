...
**Current highest-value next step:** stop treating the stale `78c5023` approval target as current. `scripts/print_expected_install_fingerprint.py` now resolves the latest component-changing candidate to exact build `840695e`, which is a real promotion-shortlist follow-on in `config_flow.py`, not docs-only churn. The next repo-side question is whether that shortlist-summary follow-on leaves any remaining mapped promotion-flow wording drift worth fixing before freeze. Only after that cut line is coherent should James be asked directly for deploy/restart approval on exact build `840695e`, rather than spending another run on unchanged fingerprint bookkeeping.
