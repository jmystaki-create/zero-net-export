...
8.  If you need to prove the running Home Assistant package matches the intended repo build, run `python3 scripts/validate_install_fingerprint.py /path/to/your/config/custom_components` in this repo. It captures `tmp/expected-install-fingerprint.json`, compares the live install, saves `tmp/install-fingerprint-compare.json`, and exits non-zero on mismatch. You can point it at the Home Assistant config directory, the `custom_components` directory, or the `custom_components/zero_net_export` directory itself. If you want the split steps for debugging, run `python3 scripts/print_expected_install_fingerprint.py --write-json tmp/expected-install-fingerprint.json`, then `python3 scripts/compare_install_fingerprint.py /path/to/your/config/custom_components --expected-json tmp/expected-install-fingerprint.json --write-json tmp/install-fingerprint-compare.json`. Compare that result with the installed package details shown in **Configure** or **Health, support, and troubleshooting**.
9.  If you need one exact manual deploy from this repo before validation, run `python3 scripts/deploy_exact_repo_build.py /path/to/your/config` or point it at `custom_components` or `custom_components/zero_net_export`. It replaces the destination component directory instead of layering files onto an older copy, creates a timestamped backup by default, and then runs `scripts/validate_install_fingerprint.py` against the deployed path before you restart Home Assistant.
...
5.  To replace a mixed manual install with one exact repo build, you can run `python3 scripts/deploy_exact_repo_build.py /path/to/your/config` before restart. It removes the old `custom_components/zero_net_export` directory at the destination, copies this repo's component directory into place, keeps a timestamped backup by default, and runs the install fingerprint validation helper against the deployed path.
...
| `scripts/deploy_exact_repo_build.py` | Replaces one target `custom_components/zero_net_export` install path with this repo's exact component directory, keeps a timestamped backup by default, and runs the fingerprint validator before restart |
