...
| `scripts/deploy_exact_repo_build.py` | Replaces one target `custom_components/zero_net_export` install path with this repo's exact component directory only after James has made the current release-target decision, accepted the closest native child-device representation, and approved exact release/deploy/restart; it can pin either repo HEAD or the latest component-changing commit before deploy, keeps a timestamped backup by default, and runs the fingerprint validator before restart |
