...
**Current highest-value next step:** deploy the now-synced exact repo build at the current repo HEAD into the confirmed live Home Assistant config path `/config`, verify the installed fingerprint matches that build, then restart Home Assistant core and continue live validation only from that synchronized install.
