...
### Fixed
- Replaced the stale archived `docs/UI_DESIGN-old.md` snapshot body with a source-of-truth pointer and moved the active entity-model command-center button description to current-focus wording, so retired release-target, ranking, and source terminology can no longer masquerade as current UI direction.
...
- Added Managed Devices status to the command-center setup check and clarified Configure Fleet activity so managed inventory/action signals stay grouped ahead of unmanaged backlog across older count labels, zero-managed fallbacks, delimiter variants including review-first/ready-next cues, and compact native Home Assistant surfaces.
