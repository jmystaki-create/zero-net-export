...
- Tightened Managed Devices workspace handoffs, Controls next-step copy, command-center setup guidance, promotion review/preset/save leads, full-list manual-add fallback copy, edit/remove copy, and review snapshots so unmanaged review, ready promotion, disabled-load counts, no-candidate, empty-fleet, manual-add, setup-checklist, and policy follow-through next steps stay workspace-first and read as direct native actions.
