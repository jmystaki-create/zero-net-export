...
### Fixed
- Clarified Home Assistant source-validation, Sensors source-role setup, and native readiness copy so source blockers, setup blockers, and progress labels consistently say `source role(s)` / `required source roles` instead of source-mapping, logical-role, or generic required-role wording.
