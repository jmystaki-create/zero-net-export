...
- Aligned Home Assistant source-role wording across the UI design/spec docs, product user stories, product/validation docs, README setup/restart guidance, bootstrap and active validation-checklist copy, validation run order, source-validation recommendations, Sensors source-health/progress headings, Controls readiness, setup checklist, setup notifications, and native readiness copy, including the remaining Sensors telemetry/detail spec ownership wording.
- Hardened stale Source blockers and source-role normalization across Managed Devices blocker handoffs, native setup notifications, command-center summaries, device-page guides, setup-check labels, setup checks, and next-action handoffs before cached or fallback text reaches Home Assistant.
...
- Clarified Configure Fleet activity so managed inventory and managed action signals stay grouped ahead of unmanaged backlog even when fallback parts arrive out of order or comma-separated, empty unmanaged inventory reads as `Unmanaged candidates: no unmanaged candidates`, global source blockers stay separate from managed/unmanaged backlog work, and fleet state remains compact for native Home Assistant surfaces.
