...
- Added Managed Devices status to the command-center setup check, clarified Configure Fleet activity so managed inventory/action signals stay grouped ahead of unmanaged backlog across older count labels, zero-managed fallbacks, delimiter variants including review-first/ready-next cues, and changed safe-mode planner copy to surface planned actions without ranking wording in compact native Home Assistant surfaces.
