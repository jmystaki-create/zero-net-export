...
### Fixed
- Fixed ZNE-594 next-step sensor state overflow at Home Assistant's 255-character limit while preserving full guidance in attributes.
...
### Validation
- Repo validation for ZNE-FR-009/ZNE-FR-010 passed focused tests (`Ran 97 tests`, OK), full discovery (`Ran 610 tests`, OK), changed-file `py_compile`, and `git diff --check`. Live HA/browser validation is part of the 0.1.110 release flow.
...
### Validation
- ZNE-591 repo validation passed focused managed-device/native-surface tests and full unittest discovery (`Ran 607 tests`, OK). Release `0.1.108` was installed in Home Assistant, fingerprint-matched before/after restart, log-checked, browser-validated for the native edit/remove rows, and live-tested to confirm removal leaves the original Home Assistant device/entity untouched.
...
### Validation
- ZNE-590 repo validation passed focused managed-device/native entity tests and full discovery; release `0.1.107` was installed in Home Assistant, fingerprint-matched before/after restart, log-checked, and browser-validated with proof for both the cleaned managed-load page and preserved original thermostat page.
...
- Rolled back the rejected `0.1.104` custom-panel candidate from the validation Home Assistant install and reverted its repo commits; ZNE-588 was reopened as a bug-only Tier 1 cleanup, with the broader native guided Tier 2 workflow kept out of the bug fix.

## [0.1.103] - 2026-05-01

### Fixed
- Made the Home Assistant Tier 1 Sensors, Controls, Managed Devices, and Diagnostics device-page launch buttons expose visible Tier 2 target links/URLs when pressed, and kept the default device-page Diagnostics card compact by hiding non-curated diagnostic telemetry by default/on upgrade.
...
- Fixed multi-service controller identity separation so existing controller devices refresh to their owning plan title (`Summer Plan`, `Winter Plan`, etc.) instead of retaining stale generic `Zero Net Export Controller` user labels; live Home Assistant deploy/restart/fingerprint validation and PNG evidence now confirm separated Summer/Winter controller rows and Winter-scoped managed devices.
...
### Changed
- Removed managed/unmanaged workspace controls and activity sources from the primary Home Assistant Zero Net Export device-info page: managed-load controls now attach to each `Managed Devices — <name>` child device, global managed/unmanaged summary actions/sensors no longer attach to the controller device, and per-load settings/actions are categorized as child-device Configuration entries.
...
- Added native device-registry support for `0.1.91`: managed-load entities now register under their own `Managed Devices — <name>` child device, and unmanaged candidates get minimal `Un Managed — <name>` diagnostic entities so Home Assistant can render them as integration-page device rows.
...
- Reworded promotion fallback preset copy from generic custom-configuration language to `Manual native settings`, replaced empty-shortlist ranking wording with neutral `surfaced candidates`, changed promotion review/preset/default-value labels to surfaced-review wording, and removed remaining `suggests` phrasing from candidate-fit summaries, keeping unmanaged-to-managed review/promotion wording explicitly inside Home Assistant's native Managed Devices workflow.
...
- Added Managed Devices status to the command-center setup check, clarified Configure Fleet activity so managed inventory/action signals stay grouped ahead of unmanaged backlog across older/case-variant count labels, zero-managed fallbacks, delimiter variants including review-first/ready-next cues, and changed safe-mode planner copy to surface planned actions without ranking wording in compact native Home Assistant surfaces.
- Tightened Managed Devices workspace handoffs, device-page promotion handoff copy, manual-add/edit/enablement/remove next steps, review snapshots, managed review notification labels, secondary review/audit blocker handoffs, and promotion review/preset/save leads so unmanaged review and ready promotion stay workspace-first instead of reading like detached secondary actions.
...
- Kept Diagnostics and operator-ready handoffs anchored to the exact native Configure and device Diagnostics paths, renamed the Diagnostics snapshot runtime next-action line/controller diagnostics payload key plus visible command-center, Managed Devices, setup-checklist, and Diagnostics button attributes away from retired ranking wording to current-action/current-focus wording, removed remaining current-release ranking wording, and capped installed-release metadata highlights so Home Assistant support surfaces stay compact.

### Planning
- Kept the detailed UI implementation map, supervisor steering, and the `0.1.89` release plan aligned on neutral current-focus / current-fleet-action wording, the clean follow-up line after the already-published `v0.1.88` release, and James's direct approval as an explicit freeze prerequisite.
...
- Fixed the native command-center diagnostic sensor so **Command center status** now reports the current status of the current native focus section instead of incorrectly echoing the section name, and made the managed-device status read from live runtime usability when available.
...
- Made the native managed-device edit and remove pickers self-orienting by showing the current fleet, the main Configure path, and the current managed-device action instead of dropping operators into bare selector-only screens.
...
- Made the native Zero Net Export command center self-orienting by showing current source status, managed-device status, policy summary, and the current focus section directly on the Configure landing screen instead of making operators infer where to go first.
