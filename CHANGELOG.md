...
- Upgraded the native fleet-console device-page action so it now shows managed and unmanaged snapshots, runtime-aware managed-device status, surfaced-candidate fit and warnings, and an explicit promotion handoff back into Configure -> Managed Devices instead of a thinner raw-entity list.
