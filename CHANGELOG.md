...
- Aligned Home Assistant source-role wording across the UI design/spec docs, product/validation docs, README setup/restart guidance, bootstrap and active validation-checklist copy, source-validation recommendations, Sensors source-health/progress headings, Controls readiness, setup checklist, setup notifications, and native readiness copy.
...
- Clarified Configure Fleet activity so managed inventory and managed action signals stay grouped ahead of unmanaged backlog even when fallback parts arrive out of order, empty unmanaged inventory reads as `Unmanaged candidates: no unmanaged candidates`, global source blockers stay separate from managed/unmanaged backlog work, and fleet state remains compact for native Home Assistant surfaces.
