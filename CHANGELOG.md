...
- Reworded Sensors and Diagnostics selector/fallback copy so native setup/support surfaces describe primary source-role choices normally, keep selector-rejection guidance in fallback-only fields, and avoid reviving old picker-bug or workaround narration.
- Tightened the default Sensors fallback guidance so the fallback block no longer describes native-dropdown workaround mechanics when no specific source blocker is present.
