...
- Reworded promotion fallback preset copy from generic `Custom` / `custom configuration` language to `Manual native settings`, replaced empty shortlist `suggested candidates` fallback copy with neutral `surfaced candidates`, and changed promotion review/preset labels from `Suggested preset` to surfaced-preset wording, keeping unmanaged-to-managed review/promotion wording explicitly inside Home Assistant's native Managed Devices workflow.
