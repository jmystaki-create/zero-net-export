...
- Clarified Configure Fleet activity so managed inventory and managed action signals stay grouped ahead of unmanaged backlog even when fallback parts arrive out of order, comma-separated, with older managed-device/unmanaged-candidate count labels, with kind inventory but no aggregate managed count, or with managed action signals but no aggregate managed count; empty unmanaged inventory reads as `Unmanaged candidates: no unmanaged candidates`, global source blockers stay separate from managed/unmanaged backlog work, and fleet state remains compact for native Home Assistant surfaces.
