...
- Tightened Managed Devices workspace handoffs, promotion review/preset/save leads, full-list manual-add fallback copy, edit/remove copy, and review snapshots so unmanaged review, ready promotion, disabled-load counts, no-candidate, empty-fleet, manual-add, and setup-checklist next steps stay workspace-first and read as direct fleet actions.
