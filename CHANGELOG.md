...
- Hardened stale Source blockers, older fallback text, and source-role/path normalization across Managed Devices blocker handoffs, native setup notifications including Controls, runtime Repairs open lists including Controls, capitalized/title-case and hyphenated setup source-mapping fallbacks, hyphenated and spaced legacy source fallback wording and case variants, command-center/device-page legacy role-blocker fallbacks, and older `Sources` handoffs, bare section handoffs, bare/shorthand, partial `Zero Net Export -> Configure ...`, case-variant, and separator-variant `Configure > ...` / `Configure -> ...` / `Configure → ...` bucket paths, already-expanded angle/unicode-arrow Configure paths, bare Configure source-role finish prompts and their case variants, older singular/plural and hyphenated `Open the source mapping(s) step` setup/support handoffs, command-center summaries, device-page guides, setup-check labels, setup checks, and next-action handoffs before cached or fallback text reaches Home Assistant, keeping primary setup blocker copy on `source blockers` instead of a separate `source-role blockers` label.
...
- Clarified Configure Fleet activity so managed inventory and managed action signals stay grouped ahead of unmanaged backlog even when fallback parts arrive out of order, comma-separated, with older managed-device/unmanaged-candidate/unmanaged-device count labels, with case-variant Managed Devices prefixes, with kind inventory but no aggregate managed count, with managed action signals but no aggregate managed count, or with unmanaged review/ready signals but no aggregate unmanaged count; empty unmanaged inventory reads as `Unmanaged candidates: no unmanaged candidates`, global source blockers stay separate from managed/unmanaged backlog work, and fleet state remains compact for native Home Assistant surfaces.
- Reworded Sensors and Diagnostics selector/fallback/opening copy so native setup/support surfaces describe primary source-role ownership, choices, and current selections normally, keep selector-rejection guidance in fallback-only fields, avoid reviving old picker-bug, workaround narration, and source-validation phrasing, and keep default fallback guidance free of native-dropdown mechanics.
- Kept Diagnostics and operator-ready handoffs anchored to the exact native Configure and device Diagnostics paths, with troubleshooting/install-validation work explicitly belonging in Diagnostics, diagnostics snapshots and support evidence labels using source-role wording, and final review/follow-up staying inside native Home Assistant paths.
