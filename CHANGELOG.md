...
- Realigned the repo working version back to the active `0.1.86` correction line so the next exact-build release candidate does not bake in a version-line mismatch before deploy. Later documented Home Assistant fingerprint rechecks show the live install has drifted again to `manifest_version=0.1.87`, so exact-build release validation is still blocked until one approved deploy/restart realigns the installed package.
