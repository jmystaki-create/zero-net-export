...
- Realigned the repo working version back to the active `0.1.86` correction line so the next exact-build release candidate does not bake in a version-line mismatch before deploy. Current documented Home Assistant fingerprint rechecks still show the live install drifting on the same six tracked component files while `manifest_version` remains `0.1.86`, so exact-build release validation stays blocked until one approved deploy/restart realigns the installed package.
