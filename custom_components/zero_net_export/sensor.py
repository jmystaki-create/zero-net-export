...
def _healthy_sources_next_step(coordinator, hass, state) -> str:
    ...
    if counts["managed_count"] == 0 and candidates:
        if review_candidate_name:
            next_step = (
                f"Open {DEVICES_CONFIGURE_PATH} to review the Managed Devices workspace, starting in the unmanaged section: {review_candidate_name}"
            )
            if ready_candidate_name and ready_candidate_name != review_candidate_name:
                next_step += (
                    f", then promote next in the Managed Devices workspace unmanaged section: {ready_candidate_name}"
                )
            return _truncate_sensor_state(next_step)
        return _truncate_sensor_state(
            f"Open {DEVICES_CONFIGURE_PATH} to review the Managed Devices workspace and promote next in the Managed Devices workspace unmanaged section: {ready_candidate_name or top_candidate_name or 'the next unmanaged candidate'}"
        )
