...
MANUAL_CANDIDATE_SHORTLIST_LABEL = "Open manual add path in the Managed Devices workspace (entity not surfaced in the shortlist)"
...
class ZeroNetExportOptionsFlow(config_entries.OptionsFlow):
    ...
    def _fleet_summary_lines(self, devices: list[dict[str, Any]]) -> list[str]:
        ...
        if active_count:
            ...
            if active_device and (not attention_count or not self._same_managed_device(active_device, attention_device)) and (not blocked_count or (blocked_device and not self._same_managed_device(active_device, blocked_device))) and (not planned_count or (planned_device and not self._same_managed_device(active_device, planned_device))):
                fleet_summary_parts.append(
    ...
    @staticmethod
    def _same_managed_device(left: dict[str, Any] | None, right: dict[str, Any] | None) -> bool:
        if not left or not right:
            return False
        left_id = str(left.get("entity_id") or left.get("key") or left.get("name") or "").strip()
        right_id = str(right.get("entity_id") or right.get("key") or right.get("name") or "").strip()
    ...
    def _candidate_snapshot_text(candidates: list[dict[str, Any]], *, limit: int = 12) -> str:
        ...
        if ready_now:
            lines.append("- Other ready unmanaged candidates:")
            lines.extend(
    ...
    def _candidate_kind_summary(candidates: list[dict[str, Any]], kind: str, *, limit: int = 6) -> str:
        ...
        if ready_now:
            lines.append(f"- Ready {kind_label} candidates:")
            lines.extend(
                f"- {build_candidate_preview(item, include_entity_id=False)}" for item in ready_now
            )
    ...
    def _candidate_summary(self, entity_id: str | None) -> dict[str, Any] | None:
        ...
        return {
            ...
            'suggested_template_label': suggested_template.label if suggested_template else 'Manual native settings',
            'suggested_template_description': suggested_template.description if suggested_template else 'Use manual native settings for this entity.',
        ...
        if missing_source_keys:
            ...
            if not source_next_step:
                if issues:
                    ...
                    source_next_step = (
                        f"Open {DEVICES_CONFIGURE_PATH} to continue in the Managed Devices workspace, review unmanaged candidate: {self._top_candidate_focus_text(primary_candidate)}"
                    )
                    if ready_candidate and ready_candidate != primary_candidate:
                        source_next_step += (
                            f", then promote ready unmanaged candidate: {self._top_candidate_focus_text(ready_candidate)}"
                        )
                    source_next_step += "."
                elif not devices:
                    source_next_step = (
                    ...
                    if review_candidate:
                        source_next_step = (
                            f"Open {DEVICES_CONFIGURE_PATH} to continue in the Managed Devices workspace, review unmanaged candidate: {candidate_focus}"
                        )
                        if ready_candidate and ready_candidate != primary_candidate:
                            source_next_step += (
                                f", then promote ready unmanaged candidate: {self._top_candidate_focus_text(ready_candidate)}"
    ...
    async def async_step_init(self, user_input=None):
        ...
        placeholders = {
            ...
            "fleet_activity_summary": format_fleet_activity_for_operator(command_center["fleet_activity_summary"]),
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                ...
                **workspace_placeholders,
                ...
                "suggested_template": str(summary.get('suggested_template_label') or 'Manual native settings'),
                "suggested_template_description": str(summary.get('suggested_template_description') or 'Use manual native settings for this entity.'),
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                ...
                **workspace_placeholders,
                ...
                "suggested_template": str((summary or {}).get('suggested_template_label') or 'Manual native settings'),
                "suggested_template_description": str((summary or {}).get('suggested_template_description') or 'Use manual native settings for this entity.'),
                "candidate_next_step": next_step,
                "promotion_path_summary": promotion_path_summary,
                "template_summary": "\n".join(
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                ...
                **workspace_placeholders,
                ...
                "device_template": selected_template.label if selected_template else "Manual native settings",
    ...
    async def async_step_policy(self, user_input=None):
        ...
        if missing_sources:
            ...
            policy_next_step = (
                f"Set Controls defaults, then use {MODE_CONTROL_PATH} or {INTEGRATION_DEVICE_PATH} to verify the current controller outcome."
            )
