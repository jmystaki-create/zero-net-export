...
def build_native_support_snapshot(coordinator: Any) -> str:
    ...
    sections = [
        ...
        *install_fingerprint_lines,
        ...
        f"- {POLICY_SECTION_LABEL}: {command_center.get('policy_path')} (live mode shortcut: {command_center.get('mode_path')})",
...
def build_native_command_center_guide_text(command_center: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Controls, including the live mode shortcut: {command_center.get('policy_path')} (device shortcut: {command_center.get('mode_path')})",
    ...
    return "\n".join(
        [
            ...
            *checklist_lines,
            ...
            f"- {SOURCES_SECTION_LABEL}: {command_center.get('sources_path')}",
            f"- {POLICY_SECTION_LABEL}, including the live mode shortcut: {command_center.get('policy_path')} (device shortcut: {command_center.get('mode_path')})",
            f"- {DEVICES_SECTION_LABEL}: {command_center.get('devices_path')}",
            f"- {SUPPORT_SECTION_LABEL}: {command_center.get('support_path')}",
            f"- Managed-device deep review: {command_center.get('detailed_management_summary')}",
