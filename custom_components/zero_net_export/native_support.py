...
def build_native_command_center_guide_text(command_center: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Runtime summary: {command_center.get('control_decision_summary')}",
            f"- Recommended section: {recommended_section}",
            f"- Recommended path: {command_center.get('recommended_path')}",
            "",
            "Basic setup paths",
            f"- Finish source mapping: {command_center.get('sources_path')}",
            f"- Tune control settings: {command_center.get('policy_path')}",
            f"- Change live control mode: {command_center.get('mode_path')}",
            f"- Open diagnostics only if setup is blocked: {command_center.get('support_path')}",
            "",
            "Not here",
            f"- Managed Devices and candidate promotion: {command_center.get('devices_path')}",
            f"- Deep diagnostics and install validation: {command_center.get('support_path')}",
        ]
    )
